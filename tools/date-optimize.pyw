# DESC: Chương trình kiểm tra và tối ưu chuỗi ngày chính xác.
...
if str(PROJECT_ROOT_FOR_IMPORT) not in sys.path:
    ...
    print(f"INFO: Added project root to sys.path: {PROJECT_ROOT_FOR_IMPORT}")
...
class TrainingApp(QMainWindow):
    ...
    def _clear_combination_selection(self):
        """Clears the combination algorithm checkboxes and resets to the initial placeholder."""
        container_layout = self.combination_layout
        if container_layout is None: # Guard against layout not existing
            trainer_logger.warning("In _clear_combination_selection: self.combination_layout is None.")
            self.combination_selection_checkboxes.clear()
            # If the layout is None, self.initial_combo_label might not be initialized or valid.
            # Mark it as None to prevent accidental use of a stale reference if hasattr passes.
            if hasattr(self, 'initial_combo_label'):
                 self.initial_combo_label = None
            return

        # Clear all widgets from the layout.
        # This will schedule them for deletion, including the old self.initial_combo_label if it was present.
        while container_layout.count():
            item = container_layout.takeAt(0)
            if item:
                widget = item.widget()
        ...
        # Always recreate self.initial_combo_label to ensure it's a valid QLabel instance
        # and add it back to the now-empty layout.
        self.initial_combo_label = QLabel("Chọn...") 
    ...
    def _populate_combination_selection(self):
        ...
        container_layout = self.combination_layout
        if container_layout is None: # Guard against layout not existing
            trainer_logger.warning("In _populate_combination_selection: self.combination_layout is None.")
            self.combination_selection_checkboxes.clear()
            return
        ...
        while container_layout.count():
            ...
            if item:
                ...
                if widget:
                    widget.deleteLater()
        self.combination_selection_checkboxes.clear() # Clear existing checkboxes data structure

        if not self.selected_algorithm_for_train:
            label = QLabel("Chưa chọn thuật toán.")
            label.setStyleSheet("font-style: italic; color: #6c757d;") # Match style of other placeholders
        ...
        if len(available) <= 1: # Only the target algo itself, or no algos
            label = QLabel("Không có thuật toán khác để kết hợp.")
        ...
        for name in available:
            if name == target: 
                continue # Don't list the target algorithm for combining with itself

            class_name = self.loaded_algorithms[name].get('class_name', name.split(' (')[0])
            chk = QCheckBox(class_name)
            chk.setToolTip(f"Thuật toán: {name}")
            container_layout.addWidget(chk)
            self.combination_selection_checkboxes[name] = chk # Store checkbox for later
            found_others = True

        if not found_others:
             # This case should be rare if len(available) > 1 and target is in available
             label = QLabel("Không có thuật toán khác để kết hợp (sau khi lọc).")
             label.setStyleSheet("font-style: italic; color: #6c757d;")
             container_layout.addWidget(label)
    ...
    # Removed duplicate _populate_combination_selection (was lines 1353-1384)
    # Removed duplicate _clear_combination_selection (was lines 1343-1351)
