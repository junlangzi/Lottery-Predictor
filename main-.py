...
import datetime
import shutil
import calendar # Keep for potential date logic, though QDate handles most
...
try:
    ...
    from PyQt5.QtCore import Qt, QTimer, QDate, QObject, pyqtSignal, QThread, QSize, QRect # Added QObject, pyqtSignal, QThread
    from PyQt5.QtGui import QFont, QPalette, QColor, QIcon, QIntValidator, QDoubleValidator, QTextCursor, QFontDatabase, QPixmap, QPainter, QBrush # Added QFontDatabase, QPixmap, QPainter, QBrush
    HAS_PYQT5 = True
    ...
    # Attempt Tkinter fallback for basic error message
    try:
        ...
        pass # If even Tkinter is missing, just print
    sys.exit(1)


# --- Astor Check (for Python < 3.9) ---
# (Keep as is)
try:
...
log_file_path = base_dir_for_log / "lottery_app_qt.log" # Path is still defined, but not used by basicConfig
logging.basicConfig(
    level=logging.DEBUG, # Keep the desired level
    format='%(asctime)s - %(name)s - %(levelname)s - [%(threadName)s] - %(message)s'
    # Removed filename=log_file_path and filemode='a' arguments
)
# --- Phần còn lại của Logging Setup giữ nguyên ---
console_handler = logging.StreamHandler(sys.stdout)
console_handler.setLevel(logging.INFO) # Log INFO level and above to console
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - [%(threadName)s] - %(message)s')
console_handler.setFormatter(formatter)
root_logger = logging.getLogger('')
# Remove existing handlers to avoid duplicates if script is re-run in some environments
for handler in root_logger.handlers[:]:
    root_logger.removeHandler(handler)
# Add ONLY the console handler
root_logger.addHandler(console_handler)
# Set root logger level (optional, basicConfig sets it but this makes it explicit)
...
# --- Import BaseAlgorithm (with fallback) ---
# (Keep as is, including the fallback dummy class)
try:
    script_dir_base = Path(__file__).parent.resolve()
    if str(script_dir_base) not in sys.path:
        sys.path.insert(0, str(script_dir_base))
    # Attempt reload if already imported (useful during development)
...
# =============================================================================
# == SECTION 1: OptimizerEmbedded (PyQt5 Version)                           ==
# =============================================================================

class OptimizerEmbedded(QWidget): # Inherit QWidget
    # Define signals for safe cross-thread communication (if needed, currently using QTimer polling)
    log_signal = pyqtSignal(str, str, str) # level, text, tag
    status_signal = pyqtSignal(str)
    progress_signal = pyqtSignal(float)
    best_update_signal = pyqtSignal(dict, tuple)
    finished_signal = pyqtSignal(str, bool, str) # message, success, reason
    error_signal = pyqtSignal(str)

    def __init__(self, parent_widget: QWidget, base_dir: Path, main_app_instance):
        super().__init__(parent_widget) # Call QWidget constructor
        self.parent_widget = parent_widget # Renamed from parent_frame
        ...
        self.main_app = main_app_instance # Keep reference to main app
        self.results_data = []
        self.loaded_algorithms = {}
        self.selected_algorithm_for_edit = None
        self.selected_algorithm_for_optimize = None
        # No tk.StringVar equivalents needed here, manage state directly
        self.editor_param_widgets = {} # Store QLineEdit/QSpinBox for editor params
        self.editor_original_params = {}
        self.optimizer_thread = None
        self.optimizer_queue = queue.Queue() # Keep using the standard queue
        ...
        self.optimizer_custom_steps = {} # Keep this structure
        self.advanced_opt_widgets = {} # Store combo/entry widgets for advanced settings
        self.can_resume = False # Simple bool, no need for tk.BooleanVar
        self.combination_selection_checkboxes = {} # Store QCheckBox for combinations
        self.current_combination_algos = []
        self.opt_start_time = 0.0
        self.opt_time_limit_sec = 0
        self.optimizer_timer = QTimer(self) # QTimer for periodic updates
        self.optimizer_timer.timeout.connect(self._check_optimizer_queue) # Check queue periodically
        self.optimizer_timer_interval = 200 # ms

        self.display_timer = QTimer(self) # QTimer for remaining time display
        self.display_timer.timeout.connect(self._update_optimizer_timer_display)
        self.display_timer_interval = 1000 # ms

        # --- PyQt5 Validators ---
        self.int_validator = QIntValidator()
        self.double_validator = QDoubleValidator()
        # Regex for custom steps: comma-separated numbers (int/float, optional sign)
        self.custom_steps_validator = QtGui.QRegularExpressionValidator(
            QtCore.QRegularExpression(r"^(?:[-+]?\d+(?:\.\d*)?(?:,\s*[-+]?\d+(?:\.\d*)?)*)?$")
        )
        # Validator for weights (allows floats)
        self.weight_validator = QDoubleValidator()
        self.dimension_validator = QIntValidator(1, 9999) # Allow only positive integers for dimensions


        self.setup_ui() # This call requires the setup_*_tab methods below
        self.load_data()
    ...
    def get_main_window(self):
        ...
        return QApplication.activeWindow() # Fallback

    # **** ĐỊNH NGHĨA CÁC HÀM setup_*_tab PHẢI NẰM Ở ĐÂY, BÊN TRONG CLASS ****
    def setup_ui(self):
        optimizer_logger.debug("Setting up optimizer embedded UI (PyQt5)...")
        main_layout = QVBoxLayout(self) # Layout chính của OptimizerEmbedded widget
        ...
        top_layout.setContentsMargins(10, 15, 10, 10) # Increased top margin
        ...
        # Removed row stretch here to let the groupbox take natural height
        # top_layout.setRowStretch(2, 1) # Push content up <-- REMOVED

        # Add the top groupbox with stretch factor 0 (takes minimum space)
        main_layout.addWidget(top_groupbox, 0)

        # --- Notebook (Tabs) ---
        self.tab_widget = QTabWidget()

        # Add the tab widget with stretch factor 1 (takes remaining space)
        ...
        # Setup content for each tab page
    ...
    def setup_select_tab(self):
        ...
        list_layout.setContentsMargins(5, 10, 5, 5) # Add top margin

        self.algo_scroll_area = QScrollArea()
        self.algo_scroll_area.setWidgetResizable(True)
        # Make scroll area background blend slightly, remove border
        ...
        # **** MODIFICATION START: Add spacing between algorithm cards ****
        self.algo_list_layout.setSpacing(8) # Adjust spacing value (e.g., 8) as needed
        # **** MODIFICATION END ****
    ...
    def setup_edit_tab(self):
        layout = QVBoxLayout(self.tab_edit) # Layout chính cho tab Chỉnh Sửa
        layout.setContentsMargins(10, 10, 10, 10)
        layout.setSpacing(10)
        # NO addStretch() here at the beginning

        # Info Frame (tên, mô tả)
        info_frame = QFrame()
        info_frame.setFrameShape(QFrame.StyledPanel)
        info_frame.setFrameShadow(QFrame.Sunken) # Add shadow for definition
        ...
        layout.addWidget(info_frame) # Add info frame

        # Splitter (tham số, giải thích)
        splitter = QSplitter(Qt.Horizontal)

        param_groupbox = QGroupBox("Tham Số Có Thể Chỉnh Sửa")
        param_outer_layout = QVBoxLayout(param_groupbox)
        param_outer_layout.setContentsMargins(5, 10, 5, 5) # Add top margin
        ...
        explain_layout.setContentsMargins(5, 10, 5, 5) # Add top margin
        ...
        # Adjust splitter initial sizes (adjust as needed)
        splitter.setSizes([splitter.width() // 2, splitter.width() // 2])

        # Add splitter with stretch factor 1, allowing it to expand vertically
        layout.addWidget(splitter, 1)

        # Button frame (Hủy, Lưu bản sao)
        ...
        layout.addWidget(button_frame) # Add button frame at the bottom
    ...
    def setup_optimize_tab(self):
        ...
        # --- Top Section (Info + Settings Grid) ---
        top_widget = QWidget()
        ...
        # Settings Grid Frame (using QHBoxLayout for the 3 columns)
        ...
        settings_layout.setHorizontalSpacing(0) # <--- Giữ nguyên hoặc giảm nếu muốn sát hơn
        ...
        self.opt_start_date_edit.setMinimumWidth(130) # Giữ nguyên hoặc giảm nếu cần
        self.opt_start_date_edit.setToolTip("Ngày bắt đầu dữ liệu dùng để kiểm tra tối ưu.")
        settings_layout.addWidget(self.opt_start_date_edit, 1, 1) # Input ở cột 1

        self.opt_start_date_button = QPushButton("📅")
        self.opt_start_date_button.setObjectName("CalendarButton")
        # self.opt_start_date_button.setFixedWidth(30) # Bỏ dòng này nếu dùng stylesheet
        ...
        settings_layout.addWidget(self.opt_start_date_button, 1, 2) # Nút ở cột 2
        ...
        self.opt_end_date_edit.setMinimumWidth(130) # Giữ nguyên hoặc giảm nếu cần
        ...
        settings_layout.addWidget(self.opt_end_date_edit, 2, 1) # Input ở cột 1

        self.opt_end_date_button = QPushButton("📅")
        self.opt_end_date_button.setObjectName("CalendarButton")
        # self.opt_end_date_button.setFixedWidth(30) # Bỏ dòng này nếu dùng stylesheet
        ...
        settings_layout.addWidget(self.opt_end_date_button, 2, 2) # Nút ở cột 2
        ...
        # --- THAY ĐỔI Ở ĐÂY ---
        # Bỏ dòng setColumnStretch(3, 1) cũ và thêm các dòng mới
        # settings_layout.setColumnStretch(3, 1) # <--- XÓA HOẶC COMMENT DÒNG NÀY

        # Thiết lập độ co giãn cho các cột
        settings_layout.setColumnStretch(0, 0)  # Cột 0 (Label "Từ/Đến ngày"): không co giãn
        settings_layout.setColumnStretch(1, 0)  # Cột 1 (QLineEdit ngày): không co giãn (sẽ lấy minimumWidth)
        settings_layout.setColumnStretch(2, 0)  # Cột 2 (Nút Lịch): không co giãn
        settings_layout.setColumnStretch(3, 1)  # Cột 3 (và các cột sau nếu có): co giãn để đẩy các cột trước sang trái
        # --- KẾT THÚC THAY ĐỔI ---

        settings_layout.setRowStretch(5, 1) # Giữ nguyên dòng này để đẩy nội dung lên trên
        settings_h_layout.addWidget(settings_groupbox, 0) # Giữ nguyên cách thêm groupbox
        ...
        # Connect dynamically in update_optimizer_ui_state
        ...
        layout.addWidget(log_groupbox, 1) # Log group takes vertical stretch
    ...
    def _setup_log_formats(self):
        ...
        self.log_formats["INFO"] = create_format(base_font, '#212529') # Dark text
        self.log_formats["DEBUG"] = create_format(base_font, '#6c757d') # Grey
        self.log_formats["WARNING"] = create_format(base_font, '#ffc107') # Yellow/Orange
        self.log_formats["ERROR"] = create_format(bold_font, '#dc3545') # Red
        self.log_formats["CRITICAL"] = create_format(bold_underline_font, '#dc3545') # Red Underline
        self.log_formats["BEST"] = create_format(bold_font, '#28a745') # Green
        self.log_formats["PROGRESS"] = create_format(base_font, '#17a2b8') # Blue/Info
        self.log_formats["CUSTOM_STEP"] = create_format(base_font, '#6f42c1') # Purple
        self.log_formats["RESUME"] = create_format(bold_font, '#17a2b8') # Blue/Info Bold
        self.log_formats["COMBINE"] = create_format(base_font, "#fd7e14") # Orange

    # --------------------------------------------------------------------------
    # --- Optimizer Logic Methods (Adapting Tkinter calls to PyQt5) ---
    # --------------------------------------------------------------------------

    def browse_data_file(self):
        optimizer_logger.debug("Browsing for optimizer data file (PyQt5)...")
        initial_dir = str(self.data_dir)
        current_path_str = self.data_file_path_label.text() # Get from label
        ...
        if filename:
            self.data_file_path_label.setText(filename) # Update label
    ...
    def load_data(self):
        ...
        self.results_data = []
        data_file_str = self.data_file_path_label.text() # Get from label

        # If label is empty or default, try finding the default file
        if not data_file_str or data_file_str == "...":
            default_path = self.data_dir / "xsmb-2-digits.json"
            if default_path.exists():
                data_file_str = str(default_path)
                self.data_file_path_label.setText(data_file_str) # Update label
            else:
                # Prompt user if default doesn't exist
                ...
                if reply == QMessageBox.Ok:
                    ...
                    data_file_str = self.data_file_path_label.text() # Get updated path
                    if not data_file_str or data_file_str == "...":
        ...
        self.data_file_path_label.setText(str(data_file_path)) # Ensure it's updated
        ...
        try:
            # --- Data Processing Logic (Identical to Tkinter version) ---
            ...
            if processed_results:
                ...
                # Set default dates for optimizer if empty
                ...
                if not self.opt_end_date_edit.text() and len(self.results_data) > 1:
                    # Default end date is one day before the last data date
    ...
    def load_algorithms(self):
        ...
        # Clear existing algorithm widgets from the layout
        ...
        # Re-add the initial label if needed later
        self.initial_algo_label = QLabel("Đang tải thuật toán...")
        self.initial_algo_label.setStyleSheet("font-style: italic; color: #6c757d;") # Info style
        self.initial_algo_label.setAlignment(Qt.AlignCenter)
        self.algo_list_layout.addWidget(self.initial_algo_label)
        ...
        # --- Algorithm Loading Logic (Identical to Tkinter version) ---
        data_copy_for_init = copy.deepcopy(self.results_data) if self.results_data else []
        cache_dir_for_init = self.calculate_dir
        loaded_algo_widgets = False # Flag to check if any algo UI was created
            ...
            try:
                ...
                if found_class:
                    try:
                        ...
                        # Call PyQt UI creation method
                        self.create_optimizer_algorithm_ui_qt(display_name, config)
                        loaded_algo_widgets = True # Mark that we created UI
                        count_success += 1
        ...
        # --- End Algorithm Loading Logic ---

        # Remove the initial "Loading..." label if algorithm widgets were added
        ...
        self.check_resume_possibility() # Update resume button state
    ...
    def create_optimizer_algorithm_ui_qt(self, display_name, config):
        ...
        # Remove initial label if it exists and this is the first algo widget
        ...
        algo_frame = QFrame()
        # **** MODIFICATION START: Apply raised effect ****
        algo_frame.setFrameShape(QFrame.StyledPanel) # Shape needed for shadow
        algo_frame.setFrameShadow(QFrame.Raised)    # Make it look raised
        algo_frame.setLineWidth(1)                  # Define border width for shadow
        algo_frame.setObjectName("CardFrame")       # Keep for potential background/border styling from main stylesheet
        # **** MODIFICATION END ****

        algo_frame_layout = QHBoxLayout(algo_frame)
        algo_frame_layout.setContentsMargins(10, 8, 10, 8) # Padding inside card
        ...
        # Use main app's font getter for consistency
        name_file_label.setFont(self.main_app.get_qfont("bold"))
        name_file_label.setStyleSheet(f"color: #212529;") # Standard dark text
        info_layout.addWidget(name_file_label)

        desc_label = QLabel(desc)
        desc_label.setWordWrap(True)
        desc_label.setFont(self.main_app.get_qfont("small")) # Use small font
        desc_label.setStyleSheet("color: #5a5a5a;") # Slightly darker grey
        desc_label.setToolTip(desc) # Use built-in tooltip
        info_layout.addWidget(desc_label)

        algo_frame_layout.addWidget(info_container, 1) # Info takes expanding space

        # Button container (Edit, Optimize)
        button_container = QWidget()
        button_layout = QVBoxLayout(button_container)
        button_layout.setContentsMargins(5, 0, 0, 0) # Padding left
        button_layout.setSpacing(4)
        button_layout.setAlignment(Qt.AlignTop) # Align buttons top

        edit_button = QPushButton("Chỉnh Sửa")
        edit_button.setObjectName("ListAccentButton") # Use existing style name for smaller buttons
        ...
        optimize_button.setObjectName("ListAccentButton") # Use existing style name
        ...
        algo_frame_layout.addWidget(button_container) # Buttons take fixed space

        # Add the created frame to the main list layout
        self.algo_list_layout.addWidget(algo_frame)
        # Store reference to the frame in the loaded_algorithms dict
        if display_name in self.loaded_algorithms:
            self.loaded_algorithms[display_name]['ui_frame'] = algo_frame

        # Tooltip helper not needed, use widget.setToolTip()
    ...
    def trigger_select_for_edit(self, display_name):
        ...
        self.selected_algorithm_for_optimize = None # Clear optimize selection
        self._clear_advanced_opt_fields()
        self._clear_combination_selection()

        self.populate_editor(display_name)
        self.tab_widget.setTabEnabled(1, True)  # Enable Edit tab
        self.tab_widget.setTabEnabled(2, False) # Disable Optimize tab
        ...
        self.check_resume_possibility() # Update resume button state (might become disabled)
    ...
    def trigger_select_for_optimize(self, display_name):
        ...
        """
        main_window = self.get_main_window()
        if not self.main_app: # Safety check
        ...
        # --- MODIFIED LOGIC for running state ---
        if self.optimizer_running:
            # Check if the user clicked the button for the ALREADY RUNNING/SELECTED algorithm
            if self.selected_algorithm_for_optimize == display_name:
                # Just switch back to the optimizer tab to view progress
                optimizer_logger.debug(f"Optimizer running/paused for '{display_name}'. Switching to Optimize tab view.")
                try:
                    # Ensure the optimize tab itself is enabled before switching
                ...
                return # Don't proceed further, just switch view

            else:
                # User clicked "Optimize" on a DIFFERENT algorithm while another optimization is running
                ...
                return # Prevent starting a new optimization
        # --- END MODIFIED LOGIC ---

        # --- Original logic executes only if optimizer is NOT running ---
        ...
        self.selected_algorithm_for_edit = None # Clear edit selection
        self._clear_editor_fields() # Clear editor tab fields

        # Populate the Optimize tab UI elements
        self.populate_optimizer_info(display_name)
        self._populate_advanced_optimizer_settings()
        self._populate_combination_selection()

        # Enable the Optimize tab and disable the Edit tab
        try:
            edit_tab_index = -1
            optimize_tab_index = -1
            for i in range(self.tab_widget.count()):
            ...
            if optimize_tab_index != -1:
                 self.tab_widget.setTabEnabled(optimize_tab_index, True)
                 self.tab_widget.setCurrentIndex(optimize_tab_index) # Switch to Optimize tab
            else:
        ...
        # Update status and load logs/check resume state
        ...
        self.check_resume_possibility() # Update resume button state based on the newly selected algorithm
    ...
    def populate_editor(self, display_name):
        ...
        # Keep color/font via stylesheet set during init
        ...
        self.editor_param_widgets = {} # Clear previous widgets map
        self.editor_original_params = copy.deepcopy(parameters)

        for name, value in parameters.items():
            # Only add editable numeric parameters
            if isinstance(value, (int, float)):
                ...
                self.editor_param_widgets[name] = param_input # Store the widget

        # Adjust layout if needed, QFormLayout usually handles this well
    ...
    def _clear_editor_fields(self):
        ...
        # Clear the QFormLayout in the editor tab
        if hasattr(self, 'edit_param_layout'):
            while self.edit_param_layout.count() > 0:
                # QFormLayout items are pairs, remove both label and field
    ...
    def cancel_edit(self):
        ...
        self.disable_edit_optimize_tabs()
        if hasattr(self, 'tab_widget'):
            self.tab_widget.setCurrentIndex(0) # Go back to Select tab
    ...
    def save_edited_copy(self):
        ...
        try:
            ...
            return
        except Exception as e: # Catch potential widget access errors
    ...
    # --- AST Modification (Keep Identical) ---
    def modify_algorithm_source_ast(self, source_code, target_class_name, new_params):
        # (Full AST modification code as provided in the original Tkinter file)
        main_window = self.get_main_window()
        ...
        class _SourceModifier(ast.NodeTransformer):
            ...
            def visit_ImportFrom(self, node):
                # Fix relative imports if they are level > 0 (e.g., from .base import BaseAlgorithm)
                if node.level > 0:
                    # Attempt to construct an absolute path based on 'algorithms' package structure
                    fixed_module_path = f"algorithms.{node.module}" if node.module else "algorithms"
                    # Specific fix for 'from .base import ...'
                    if node.module == 'base':
                        node.module = 'algorithms.base' # Make it absolute relative to project root
                    ...
                    # General fix for other relative imports like 'from .utils import ...'
                    elif node.module:
                        node.module = fixed_module_path
                        node.level = 0
                        self.imports_modified = True
                        optimizer_logger.debug(f"AST Fix: Changed 'from .{node.module}' to 'from {fixed_module_path}'")
                    # If node.module is None (e.g., from . import something), it's trickier, maybe ignore or log error
                    # For now, we only handle cases where node.module is set
            ...
            def visit_Assign(self, node):
                 # Looking for assignments like `self.config = {...}` inside the target class's __init__
                if self.in_target_init and len(node.targets) == 1:
                    ...
                    if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) and target.value.id == 'self' and target.attr == 'config':
                         # If it is, visit the value (the dictionary) to potentially modify it
                         node.value = self.visit(node.value)
                         return node # Return the potentially modified assignment node
                 # For other assignments, just continue visiting
                return self.generic_visit(node)
            def visit_Dict(self, node):
                # Only modify dictionaries if we are inside the target class's __init__
                if not self.in_target_init:
                    return self.generic_visit(node)

                param_key_index = -1
                param_value_node = None
                # Find the 'parameters' key within this dictionary
                try:
                    if node.keys: # Ensure keys list is not None
                        for i, key_node in enumerate(node.keys):
                            # Handle Python 3.8+ (ast.Constant) and older (ast.Str) for the key 'parameters'
                            is_param_key = (isinstance(key_node, ast.Constant) and isinstance(key_node.value, str) and key_node.value == 'parameters') or \
                                           (hasattr(ast, 'Str') and isinstance(key_node, ast.Str) and key_node.s == 'parameters') # Check if ast.Str exists
                     ...
                     return self.generic_visit(node) # Fallback to generic visit on error


                # If 'parameters' key is found and its value is another Dict
                if param_key_index != -1 and isinstance(param_value_node, ast.Dict):
                    new_keys = []
                    new_values = []
                    modified_in_subdict = False

                    # Store original key-value nodes from the 'parameters' sub-dictionary
                    original_param_nodes = {}
                    if param_value_node.keys: # Check if keys exist
                        for k, v in zip(param_value_node.keys, param_value_node.values):
                            param_name_str = None
                            if isinstance(k, ast.Constant) and isinstance(k.value, str): # Py 3.8+
                    ...
                    # Update values for parameters specified in self.params_to_update
                    for param_name, new_value in self.params_to_update.items():
                        if param_name in original_param_nodes:
                            p_key_node, p_val_node = original_param_nodes[param_name] # Get original key/value nodes
                            new_val_node = None

                            # --- Convert Python value to AST node ---
                            # Python 3.8+ uses ast.Constant for simple types
                            if sys.version_info >= (3, 8):
                                if isinstance(new_value, (int, float)):
                                    # Handle negative numbers using UnaryOp
                                    if new_value < 0:
                            ...
                            # Python < 3.8 uses ast.Num, ast.Str, ast.NameConstant
                            else:
                                if isinstance(new_value, (int, float)):
                                    new_val_node = ast.Num(n=new_value) # Handles sign directly
                                elif isinstance(new_value, str):
                            ...
                            if new_val_node is not None:
                                ...
                                new_values.append(new_val_node) # Add new value node
                                modified_in_subdict = True
                            else:
                                # If conversion failed (shouldn't normally happen for supported types), keep original
                                new_keys.append(p_key_node)
                                new_values.append(p_val_node)
                        # else: parameter from update list wasn't in the original dict, ignore it.
                ...
                # Continue visiting other nodes within the dictionary if modification didn't happen at this level
                return self.generic_visit(node)
        ...
        try:
            ...
            # Use astor if available (for Python < 3.9)
            elif HAS_ASTOR:
    ...
    # --- Populate Combination Selection ---
    def _populate_combination_selection(self):
        ...
        for algo_name in available_algos:
            ...
            chk.setToolTip(algo_name) # Show full name on hover
    ...
    # --- Populate Advanced Optimizer Settings ---
    def _populate_advanced_optimizer_settings(self):
        ...
        if display_name not in self.loaded_algorithms:
            ...
            error_label.setStyleSheet("color: #dc3545;") # Danger color
        ...
        header_layout.setContentsMargins(5, 5, 5, 10) # Add padding
        header_layout.addWidget(QLabel("Tham số"), 2) # Stretch factor 2
        header_layout.addWidget(QLabel("Giá trị gốc"), 1) # Stretch factor 1
        header_layout.addWidget(QLabel("Chế độ"), 1) # Stretch factor 1
        header_layout.addWidget(QLabel("Bước (+/-) cách bởi dấu phẩy"), 3) # Stretch factor 3
        ...
        for name, value in numeric_params.items():
            ...
            param_layout.setContentsMargins(5, 2, 5, 2) # Reduced vertical padding

            if name not in self.optimizer_custom_steps:
                # Initialize state if not present (use dict, not tk.StringVar)
                self.optimizer_custom_steps[name] = {'mode': 'Auto', 'steps': [], 'str_value': ""}

            param_state = self.optimizer_custom_steps[name]

            # Widgets for this parameter row
            param_label = QLabel(name)
            param_layout.addWidget(param_label, 2) # Stretch factor 2

            value_label = QLabel(f"{value:.4g}" if isinstance(value, float) else str(value))
            value_label.setStyleSheet("color: #6c757d;") # Secondary color
            param_layout.addWidget(value_label, 1) # Stretch factor 1

            mode_combo = QComboBox()
            mode_combo.addItems(["Auto", "Custom"])
            mode_combo.setCurrentText(param_state['mode'])
            mode_combo.setFixedWidth(80)
            param_layout.addWidget(mode_combo, 1) # Stretch factor 1

            steps_entry = QLineEdit(param_state.get('str_value', '')) # Get saved string value
            steps_entry.setValidator(self.custom_steps_validator)
            steps_entry.setEnabled(param_state['mode'] == 'Custom') # Enable based on mode
            param_layout.addWidget(steps_entry, 3) # Stretch factor 3
    ...
    def _on_step_mode_change(self, param_name, mode_combo_widget, steps_entry_widget):
        ...
        if param_name in self.optimizer_custom_steps:
            ...
            if is_custom:
                ...
                # Trigger validation/update based on current text
                self._update_custom_steps(param_name, steps_entry_widget)
            else:
                # Clear error styling if switching back to Auto
                steps_entry_widget.setStyleSheet("") # Reset stylesheet

    def _validate_custom_steps_input_bool(self, text):
        """Internal helper to check custom steps syntax purely based on regex."""
        if not text: return True # Empty is valid
        # Use the regex from the validator
    ...
    def _update_custom_steps(self, param_name, steps_entry_widget):
        ...
        error_style = "QLineEdit { border: 1px solid #dc3545; }" # Red border for error

        if param_name in self.optimizer_custom_steps:
            # Store the raw string value
            self.optimizer_custom_steps[param_name]['str_value'] = steps_str

            # Only parse if mode is Custom
            if self.optimizer_custom_steps[param_name]['mode'] == 'Custom':
                ...
                if is_valid_syntax and steps_str:
                    try:
                        ...
                        for part in steps_str.split(','):
                            part = part.strip()
                            if part:
                                num_val = float(part) # Allow float parsing initially
                                if is_original_int:
                                    # If original is int, step must also be an integer
                                    ...
                                    # Original is float, step can be float
                ...
                elif not is_valid_syntax and steps_str: # Invalid syntax and not empty
                    parse_error = True
                    parsed_steps = []
                else: # Empty string
                    parse_error = False # Empty is not an error per se, just no steps
                    parsed_steps = []
                ...
                if parse_error:
                    ...
                    steps_entry_widget.setStyleSheet("") # Clear style if valid

            else: # Mode is Auto
                self.optimizer_custom_steps[param_name]['steps'] = [] # Ensure steps are empty for Auto mode
                steps_entry_widget.setStyleSheet("") # Clear any error style
    ...
    def populate_optimizer_info(self, display_name):
        if display_name in self.loaded_algorithms:
            ...
            # Style is set during init
        else:
            self.opt_algo_name_label.setText("Lỗi: Không tìm thấy thuật toán")
            self.opt_algo_name_label.setStyleSheet("color: #dc3545;") # Error color
    ...
    def start_optimization(self, initial_params=None, initial_score_tuple=None, initial_combination_algos=None):
        ...
        # Validate dates and time limit
        start_d, end_d, time_limit_min = self._validate_common_opt_settings_qt()
        if start_d is None: # Validation failed, message shown inside helper
            return

        # Finalize custom steps config (handles potential invalid inputs)
        ...
        if has_invalid_custom_steps:
            # Optionally refresh the UI to show corrected modes/styles
            self._populate_advanced_optimizer_settings()
    ...
    def resume_optimization_session(self):
        ...
        # Find latest result/state file (logic is identical)
        ...
        try:
            # --- Load and Validate Data from JSON (Identical Logic) ---
            ...
            # Check parameter mismatch (logic identical)
            ...
            # Check missing combination algorithms (logic identical)
            ...
            # Call start_optimization with loaded initial state
    ...
    def _validate_common_opt_settings_qt(self, check_dates=True):
        ...
        if check_dates:
            ...
            # Check if selected range is valid w.r.t loaded data
            # End date must be strictly less than the last data date
        ...
        try:
            ...
            if time_limit_min <= 0:
                 # Should be prevented by spinbox range, but check anyway
                 QMessageBox.critical(main_window, "Lỗi Thời Gian", "Thời gian tối ưu tối đa phải lớn hơn 0 phút.")
                 return None, None, None
        except Exception as e: # Catch potential errors getting value
    ...
    def _finalize_custom_steps_config_qt(self, original_params):
        ...
        # Iterate through the parameters that *have* advanced widgets created
        for name, widgets in self.advanced_opt_widgets.items():
            mode_combo = widgets.get('mode_combo')
            steps_entry = widgets.get('steps_entry')

            if not mode_combo or not steps_entry: continue # Skip if widgets not found

            mode = mode_combo.currentText()
            steps_str = steps_entry.text().strip()
            parsed_steps = []
            is_final_mode_custom = False
            param_state = self.optimizer_custom_steps.get(name, {'mode': 'Auto', 'steps': []}) # Get current internal state

            if mode == 'Custom':
                # Use the already validated internal state if possible
                # Re-validate the string just in case
                is_valid_syntax = self._validate_custom_steps_input_bool(steps_str)
                ...
                if is_valid_syntax and steps_str:
                    try:
                        ...
                        if temp_parsed:
                             parsed_steps = sorted(list(set(temp_parsed)))
                             is_final_mode_custom = True # Successfully parsed custom steps
                        else:
                             # Valid syntax but resulted in empty list (e.g., "0, 0")
                ...
                # If custom mode failed validation or parsing, force mode back to Auto internally
                if has_invalid_custom_steps and is_final_mode_custom == False:
                    param_state['mode'] = 'Auto'
                    param_state['steps'] = []


            # Store the final configuration for this parameter
        ...
        # Show warning message if any custom steps were defaulted to Auto
        if has_invalid_custom_steps:
    ...
    def _start_optimization_worker_thread(self, display_name, start_date, end_date, time_limit_min,
        ...
        self.current_optimization_log_path = self.current_optimize_target_dir / "optimization_qt.log" # Changed log name

        # --- Clear and Prepare Log Display ---
        if hasattr(self, 'opt_log_text'):
            self.opt_log_text.clear() # Clear previous log content
            if not is_resuming:
        ...
        # Store date range for potential saving
        ...
        # Show and initialize time remaining display
        ...
        self.update_optimizer_ui_state() # Update button enable/disable states

        # --- Start Thread ---
        optimizer_logger.info("Creating and starting optimizer worker thread.")
        self.optimizer_thread = threading.Thread(
            target=self._optimization_worker, # Keep using the same worker function
        ...
        # Start timers for queue checking and display updates
    ...
    def pause_optimization(self):
        if self.optimizer_running and not self.optimizer_paused:
            ...
            self._save_optimization_state(reason="paused") # Save state on pause
    ...
    def stop_optimization(self, force_stop=False):
        ...
        if self.optimizer_running:
            ...
            if confirmed:
                self._save_optimization_state(reason="stopped") # Save best result so far
                self.optimizer_stop_event.set()

                # Update UI immediately to indicate stopping process
                ...
                # If paused, clear the pause event to allow the worker to check the stop event
    ...
    def update_optimizer_ui_state(self):
        ...
        pause_style_obj_name = "WarningButton" # Default pause style
        ...
        if self.optimizer_running:
            ...
            if self.optimizer_paused:
                ...
                # Keep Warning style for Resume button
            else: # Running, not paused
                pause_enabled = True
                pause_text = "Tạm dừng"
                pause_callback = self.pause_optimization
        else: # Not running (idle)
            start_enabled = (self.selected_algorithm_for_optimize is not None)
            resume_enabled = self.can_resume # Use the boolean flag directly
            stop_enabled = False
            pause_enabled = False
            pause_text = "Tạm dừng"
            pause_callback = self.pause_optimization # Reset callback

            # Reset status/time display
        ...
        if hasattr(self, 'opt_pause_button'):
            ...
            self.opt_pause_button.setObjectName(pause_style_obj_name) # Apply style name
            # Disconnect previous connection before connecting new one
            ...
            self.main_app.apply_stylesheet() # Reapply stylesheet to update button look if needed

        # --- Settings Widgets State ---
        ...
        for name, widgets in self.advanced_opt_widgets.items():
            ...
            if steps_entry:
                # Enable steps entry only if mode is Custom AND optimizer is not running
    ...
    def _update_optimizer_timer_display(self):
        ...
        if seconds_left >= 0:
            ...
            time_str = "00:00" # Should ideally not happen if logic is correct
        ...
        try:
            while not self.optimizer_queue.empty():
                ...
                if msg_type == "log":
                    ...
                    # self._log_to_optimizer_display("BEST", f"New best: {self.current_best_params} -> Score: {self.current_best_score_tuple}", "BEST")
                    ...
                    if success:
                        ...
                        log_level, log_tag_prefix = "BEST", "[HOÀN TẤT]" # Still potentially useful result
                    ...
                    self.check_resume_possibility() # Check if a resumable state exists now
                    self.update_optimizer_ui_state() # Reset buttons, hide timer, etc.

                    self.optimizer_thread = None # Clear thread reference
                    ...
                    self.optimizer_running = False
                    self.update_optimizer_ui_state() # Reset UI to idle
                    return # Stop checking queue

        except queue.Empty:
            pass # No messages this time
        except Exception as e:
            optimizer_logger.error(f"Error processing optimizer queue: {e}", exc_info=True)
            # Stop timers on queue processing error
            if self.optimizer_timer.isActive(): self.optimizer_timer.stop()
            if self.display_timer.isActive(): self.display_timer.stop()
            self.optimizer_running = False
            self.update_optimizer_ui_state() # Reset UI

        # # QTimer handles rescheduling automatically if it wasn't stopped
        # # No need for manual rescheduling like root.after
    ...
    def _log_to_optimizer_display(self, level, text, tag=None):
        ...
        try:
            ...
            if hasattr(self, 'opt_log_text'):
                ...
                if display_tag == "CRITICAL": display_tag = "ERROR" # Map critical to error for display
                log_format = self.log_formats.get(display_tag, self.log_formats["INFO"]) # Get QTextCharFormat

                # Move cursor to the end
                cursor = self.opt_log_text.textCursor()
                cursor.movePosition(QTextCursor.End)
                # **** APPLY FORMAT TO THE ENTIRE LINE ****
                # Insert the full line with the determined format
                full_log_line = f"{timestamp} [{level.upper()}] {text}\n"
                cursor.insertText(full_log_line, log_format)

                # self.opt_log_text.setTextCursor(cursor) # Set cursor seems redundant after insertText
                self.opt_log_text.ensureCursorVisible() # Scroll to the end

                # Write to the physical log file (remains the same)
    ...
    # --- Worker Function (_optimization_worker) ---
    # This function's core logic (the optimization loop, calling algorithms,
    # checking events, calculating scores) remains identical.
    # The only changes are how it communicates back to the UI thread:
    # Instead of directly updating UI variables, it puts messages into
    ...
    def _optimization_worker(self, target_display_name, start_date, end_date, time_limit_sec,
        ...
        # --- Queue Helper Functions (remain the same internally) ---
        ...
        finish_reason = "completed" # Default finish reason
        try:
            # --- Initial Setup ---
            if target_display_name not in self.loaded_algorithms:
            ...
            # Ensure source code is read correctly before modification attempts
            try:
                source_code = original_path.read_text(encoding='utf-8')
            except Exception as read_err:
                raise RuntimeError(f"Failed to read source code for {original_path.name}: {read_err}")

            target_dir = self.current_optimize_target_dir # Already created
            ...
            # --- Wrappers and Score Function ---
            # Wrapper remains the same conceptually
            def run_combined_perf_test_wrapper(target_params_test, combo_names, start_dt, end_dt):
                 # Note: The run_combined_performance_test method itself uses queue_error for logging errors
            ...
            def get_primary_score(perf_dict):
                 ...
                 # Prioritize Top3, then Top5, then Top1, then negatively by repetition
                 return (perf_dict.get('acc_top_3_pct',0.0),
                         perf_dict.get('acc_top_5_pct',0.0),
                         perf_dict.get('acc_top_1_pct',0.0),
                         -perf_dict.get('avg_top10_repetition',100.0)) # Negate repetition

            # --- Initialize Best State ---
            current_best_params = {}
            current_best_perf = None
            # Use initial value if resuming, otherwise default bad score
            ...
            if is_resuming:
                ...
                if self.optimizer_stop_event.is_set():
                    ...
                    # Send finished signal immediately as we are stopping
                ...
                if recalc_perf is not None:
                    current_best_perf = recalc_perf # Store the recalculated performance data
                    recalc_score = get_primary_score(recalc_perf)
                    # Compare recalculated score with the loaded score
                    ...
                    # Send the potentially updated best state back to UI
                    queue_best_update(current_best_params, current_best_score_tuple)
                else:
                    # Recalculation failed (and wasn't stopped)
                    ...
                    return # Exit worker on critical resume error
            else:
                 ...
                 # Check if stopped during initial test
                 if self.optimizer_stop_event.is_set():
                     finish_reason = "stopped"
                 ...
                 if initial_perf is None:
                     ...
                     return # Exit worker on critical initial error

                 # Initialize best state with original parameters and performance
                 ...
                 # Send initial best state to UI
                 queue_best_update(current_best_params, current_best_score_tuple)


            # --- Optimization Loop ---
            MAX_ITERATIONS_PER_PARAM_AUTO = 10 # Max steps in one direction for auto mode
            STALL_THRESHOLD = 2               # Stop direction if no improvement for this many steps
            MAX_FULL_CYCLES = 5               # Max times to iterate through all parameters
            steps_done = 0                     # Counter for total performance tests run
            ...
            for cycle in range(MAX_FULL_CYCLES):
                ...
                for param_idx, param_name in enumerate(param_names_ordered):
                    # Checkpoints for stop/pause/time limit
                    ...
                    if finish_reason == "stopped": break # Check again after pause potentially resolved by stop
                    elapsed_time = time.time() - start_time
                    if elapsed_time >= time_limit_sec: finish_reason = "time_limit"; break

                    # --- Get Optimization Config for this Parameter ---
                    ...
                    # Use the current best value as the starting point for this parameter's turn
                    ...
                    if mode == 'Custom' and custom_steps:
                        ...
                        best_value_this_param = current_best_params[param_name] # Track best value found *for this parameter* in this turn

                        # Test each custom step in both positive and negative directions
                        for step_sign in [1, -1]: # Try adding and subtracting the step
                            for step_val in custom_steps:
                                # Checkpoints within inner loop
                                if self.optimizer_stop_event.is_set(): finish_reason="stopped"; break
                                if time.time() - start_time >= time_limit_sec: finish_reason="time_limit"; break
                                if step_val == 0: continue # Skip zero step

                                # Prepare parameters for testing this step
                                test_params = current_best_params.copy()
                                # Calculate the new value based on the best found *so far* for this param turn
                                new_value = best_value_this_param + (step_sign * step_val)
                                # Apply formatting/type conversion
                                ...
                                steps_done += 1
                                queue_progress(min(0.95, (time.time() - start_time) / time_limit_sec)) # Update progress bar

                                # Check if stopped during the test
                                if self.optimizer_stop_event.is_set(): finish_reason="stopped"; break
                                ...
                                if perf_result is not None:
                                    ...
                                    if new_score > current_best_score_tuple:
                                        ...
                                        best_value_this_param = new_value # Update best for *this param*
                                        queue_best_update(current_best_params, current_best_score_tuple) # Notify UI
                                        params_changed_in_cycle = True # Mark change in cycle
                                    # else: No improvement with this step
                                else:
                                    # Performance test failed for this step (and wasn't stopped)
                                    queue_log("WARNING", f"  -> Lỗi Test {sign_char} custom {param_name}={test_params[param_name]}.", tag="WARNING")
                            # Checkpoints after inner step loop
                            if finish_reason in ["stopped", "time_limit"]: break
                        # Checkpoints after direction loop
                        if finish_reason in ["stopped", "time_limit"]: break

                    # --- Auto Step Mode ---
                    else: # Mode is 'Auto' or Custom mode had no valid steps
                        # Determine initial step size (simple percentage or minimum)
                        step_base = abs(original_value_for_turn) * 0.05 # 5% of current value
                        if not is_float: # Integer parameter
                            step = max(1, int(round(step_base))) # At least 1
                        else: # Float parameter
                            if abs(original_value_for_turn) > 1e-9: # Avoid tiny steps if value is near zero
                                step = max(1e-6, step_base) # Minimum step for floats
                            else:
                                step = 0.001 # Default small step if value is zero or very small
                        ...
                        # Explore in both positive and negative directions
                        for direction_sign in [1, -1]: # +1 for increase, -1 for decrease
                            no_improve_streak = 0
                            # Start exploring from the *current global best* parameter set
                            params_at_dir_start = current_best_params.copy()
                            current_val_dir = params_at_dir_start[param_name] # Value to modify in this direction
                            dir_char = '+' if direction_sign > 0 else '-'; dir_text = 'tăng' if direction_sign > 0 else 'giảm'

                            # Try multiple steps in this direction
                            for i in range(MAX_ITERATIONS_PER_PARAM_AUTO):
                                # Checkpoints
                                if self.optimizer_stop_event.is_set(): finish_reason="stopped"; break
                                if time.time() - start_time >= time_limit_sec: finish_reason="time_limit"; break

                                # Calculate next value in this direction
                                current_val_dir += (direction_sign * step)
                                test_params = params_at_dir_start.copy() # Use copy to avoid accumulation issues
                                ...
                                if perf_result is not None:
                                    ...
                                    if new_score > current_best_score_tuple:
                                        ...
                                        # **Critical:** Update the global best params AND the starting point for the *next step* in this direction
                                        current_best_params = test_params.copy()
                                        params_at_dir_start = test_params.copy() # Update base for next step in this dir
                                        current_val_dir = test_params[param_name] # Update current value for this dir

                                        current_best_perf = perf_result
                                        current_best_score_tuple = new_score
                                        queue_best_update(current_best_params, current_best_score_tuple) # Notify UI
                                        params_changed_in_cycle = True
                                        no_improve_streak = 0 # Reset streak on improvement
                                    else:
                                        # No improvement, increase streak
                                        no_improve_streak += 1
                                        queue_log("DEBUG", f"  -> Không cải thiện ({dir_char} auto) {param_name}={test_params[param_name]:.4g}. Streak: {no_improve_streak}")

                                    # Stop exploring this direction if stalled
                                    ...
                                    # Performance test failed (and not stopped)
                                    no_improve_streak += 1 # Count failure as non-improvement
                                    ...
                                    if no_improve_streak >= STALL_THRESHOLD:
                                        ...
                                        break # Stop direction if test fails repeatedly

                            # Checkpoints after inner auto loop
                            if finish_reason in ["stopped", "time_limit"]: break
                        # Checkpoints after direction loop
                        if finish_reason in ["stopped", "time_limit"]: break
                    # Checkpoints after parameter optimization (Auto or Custom)
                    if finish_reason in ["stopped", "time_limit"]: break
                # --- End Parameter Loop ---

                if finish_reason in ["stopped", "time_limit"]: break # Break outer cycle loop if needed

                # Check if any parameter improved in this full cycle
                if not params_changed_in_cycle and cycle > 0: # Only check after the first cycle
                    queue_log("INFO", f"Không có cải thiện nào trong chu kỳ {cycle + 1}. Dừng tối ưu.", tag="PROGRESS")
                    finish_reason = "no_improvement"
                    break
            # --- End Optimization Cycle Loop ---

            # --- Finalization ---
            queue_progress(1.0) # Ensure progress reaches 100%
            final_message = ""
            ...
            elif finish_reason == "no_params": final_message = "Thuật toán không có tham số để tối ưu." # Already logged earlier
            ...
            elif finish_reason == "critical_error": final_message = "Lỗi nghiêm trọng trong worker." # Set by except block below
            else: final_message = "Tối ưu hoàn tất."

            # --- Log and Save Best Result ---
            # Check if we have a valid best result to log/save
            # (exclude cases where optimization couldn't even start properly)
            ...
            if can_log_or_save:
                ...
                try:
                    ...
                    final_save_data = {
                        ...
                        "performance": current_best_perf if current_best_perf else "N/A", # Include full perf dict
                        "score_tuple": list(current_best_score_tuple), # Save the score tuple used for comparison
            ...
            # --- End Save Results ---
                ...
                # Case where optimization ran but found nothing better than initial/resumed state
            ...
            # Determine overall success flag for the finished signal
            # Consider successful if it completed normally, or by time limit/no improvement (as these imply results were found)
            is_successful_run = finish_reason in ["completed", "time_limit", "no_improvement"] and can_log_or_save

            # Send finished signal to UI thread
            queue_finished(final_message, success=is_successful_run, reason=finish_reason)

        except Exception as worker_err:
            # Catch-all for unexpected errors within the worker
            ...
            # Send a finished signal indicating failure
    ...
    def _save_optimization_state(self, reason="unknown"):
        ...
        try:
            ...
            target_dir.mkdir(parents=True, exist_ok=True) # Should exist, but double-check
            ...
            # Use the stored date range strings
            ...
            self.check_resume_possibility() # Update resume button state after saving
        except Exception as e:
    ...
    # --- Performance Test (run_combined_performance_test) ---
    # Core logic is identical. It calls other internal methods like
    # modify_algorithm_source_ast and _import_and_instantiate_temp_algo,
    # which are also kept the same. It uses threading events.
    # It communicates errors via queue_error.
    def run_combined_performance_test(self, target_display_name, target_algo_source, target_class_name,
        ...
        # Queue error function (local helper for this method)
        ...
        try:
            ...
            # --- Load Target Algorithm Instance ---
            try:
                # Modify source with test parameters
                ...
                # Ensure necessary __init__.py files exist for importlib
                ...
                # Define module name relative to the base 'optimize' directory
                # e.g., optimize.my_algo_stem.temp_perf_target_MyAlgo_12345
            ...
            # --- Load Combination Algorithm Instances ---
            worker_logger.debug(f"Loading {len(combination_algo_display_names)} combination algorithms.")
            data_copy_for_combo = copy.deepcopy(self.results_data) # Use a fresh copy
            for combo_name in combination_algo_display_names:
                ...
                try:
                     ...
                     # Instantiate directly using the class from the loaded data
                     # Pass a fresh copy of data and the correct cache dir
                     ...
                     # Log error but continue with other combination algorithms
                     worker_logger.error(f"Failed loading COMBO instance {combo_name} for perf test: {combo_load_err}", exc_info=True)
                     if combo_name in combo_instances: del combo_instances[combo_name] # Clean up failed instance

            # --- Performance Calculation Loop (Identical Logic) ---
            ...
            for i, r in enumerate(sorted_results_for_cache):
                 history_cache[r['date']] = sorted_results_for_cache[:i] # Store results *before* date r
            ...
            all_top_10_combined_numbers = [] # To calculate repetition
            ...
            while current_date <= test_end_date:
                ...
                hist_data = history_cache.get(predict_date) # Get history *before* predict_date

                # Check if we have necessary data for this day
                if actual_result_dict is None or hist_data is None:
                    # worker_logger.debug(f"Skipping {predict_date}: Missing actual ({actual_result_dict is None}) or history ({hist_data is None}) for {check_date}.")
                    current_date += datetime.timedelta(days=1)
                    continue

                # Extract actual winning numbers for the check_date
                actual_numbers_set = set()
                # Use the target instance's method (it should be loaded)
                if target_instance:
                    actual_numbers_set = target_instance.extract_numbers_from_dict(actual_result_dict)
                else:
                    # This shouldn't happen if target loaded correctly, but handle defensively
                ...
                if not actual_numbers_set:
                    # worker_logger.warning(f"Could not extract actual numbers for {check_date} from dict: {actual_result_dict}")
                    stats['errors'] += 1 # Count as error if no numbers found
                    current_date += datetime.timedelta(days=1)
                    continue

                # --- Run Predictions for the Day ---
                all_predictions_for_day = {}
                hist_copy_for_day = copy.deepcopy(hist_data) # Use copy for prediction

                # Target prediction
                if target_instance:
                    try:
                        # worker_logger.debug(f"Predicting target {target_class_name} for {predict_date}")
                        ...
                        all_predictions_for_day[target_display_name] = {} # Empty dict on error
                        stats['errors'] += 1
                else:
                    all_predictions_for_day[target_display_name] = {} # Should not happen
                    stats['errors'] += 1

                # Combination predictions
                for combo_name, combo_inst in combo_instances.items():
                    try:
                        # worker_logger.debug(f"Predicting combo {combo_name} for {predict_date}")
                        ...
                        all_predictions_for_day[combo_name] = {} # Empty dict on error
                        stats['errors'] += 1

                # --- Combine Scores ---
                # Initialize combined scores (deltas) for the day
                ...
                for algo_name, scores_dict in all_predictions_for_day.items():
                    if not isinstance(scores_dict, dict) or not scores_dict:
                        # worker_logger.debug(f"Skipping empty/invalid result from {algo_name} for {predict_date}")
                        continue # Skip empty or invalid results

                    valid_algo_count += 1
                    # Add deltas (assuming scores_dict contains deltas)
                    for num_str, delta_val in scores_dict.items():
                        ...
                        if isinstance(num_str, str) and len(num_str)==2 and num_str.isdigit():
                            try:
                                ...
                                stats['errors'] += 1
                        # else: worker_logger.debug(f"Skipping invalid key '{num_str}' from {algo_name}")


                # If no algorithms provided valid scores, skip day
                if valid_algo_count == 0:
                    # worker_logger.warning(f"No valid algorithm scores to combine for {predict_date}")
                    stats['errors'] += 1
                    current_date += datetime.timedelta(days=1)
                    continue

                # Convert raw combined scores (deltas) to list of tuples for sorting
                combined_scores_list = []
                base_score = 100.0 # Assuming a base score if combining deltas
                for num_str, delta in combined_scores_raw.items():
                     try:
                         # Final score = base + delta
                         ...
                         stats['errors'] += 1
                         # Optionally add with a default score or skip
                ...
                # Add top 10 to list for repetition calculation
                all_top_10_combined_numbers.extend(list(pred_top_10))

                # Increment tested days count *only if* processing was successful for the day
            ...
            if total_tested > 0:
                ...
                if all_top_10_combined_numbers:
                    ...
                    # Store top 5 most common numbers and their counts
                ...
                # Set percentages to 0 if no days were tested
            ...
            return stats # Return the calculated statistics dictionary
    ...
    def _import_and_instantiate_temp_algo(self, temp_filepath, temp_module_name, class_name_hint):
        ...
        module_obj = None
        try:
            # Remove if already exists (important for re-runs)
            ...
            if not temp_class or not inspect.isclass(temp_class) or not issubclass(temp_class, BaseAlgorithm):
                 ...
                 for name, obj in inspect.getmembers(module_obj):
                     # Check if it's a class, subclass of BaseAlgorithm, not BaseAlgorithm itself,
                     # and defined directly within the temporary module
                     if inspect.isclass(obj) and issubclass(obj, BaseAlgorithm) and obj is not BaseAlgorithm and obj.__module__ == temp_module_name:
                         ...
                         break # Found the first valid class
            ...
            # Provide necessary data (deep copy results, cache dir)
    ...
    def find_latest_successful_optimization(self, success_dir: Path, algo_stem: str):
        ...
        if success_dir.is_dir():
            try:
                ...
                for f_path in json_files:
                    try:
                        file_timestamp = 0
                        try: # Try parsing timestamp from filename first
                            parts = f_path.stem.split('_')
                            # Assuming format optimized_STEM_perf_YYYYMMDD_HHMMSS.json
                            file_ts_str = f"{parts[-2]}_{parts[-1]}" # YYYYMMDD_HHMMSS
        ...
        # If a successful optimization file was found, return it
        if latest_file:
            optimizer_logger.info(f"Latest successful optimization found: {latest_file.name}")
            return latest_file, latest_data

        # If no success file, check for a saved state file as fallback
    ...
    def check_resume_possibility(self, event=None):
        ...
        if target_algo and target_algo in self.loaded_algorithms:
            ...
            success_dir = optimize_target_dir / "success"
            # Check for latest success or state file
        ...
        self.can_resume = can_resume_flag # Update the boolean flag

        # Update UI state (especially the resume button) if optimizer is not running
        if not self.optimizer_running:
            self.update_optimizer_ui_state()

    def update_status(self, message):
        """Updates the status label, logs the message."""
        # Log the message regardless of UI state
        optimizer_logger.info(f"Optimizer Status: {message}")
        if hasattr(self, 'opt_status_label'):
             self.opt_status_label.setText(f"Trạng thái: {message}")
             # Optionally change style based on message content
             lower_msg = message.lower()
             if "lỗi" in lower_msg or "fail" in lower_msg:
                 self.opt_status_label.setStyleSheet("color: #dc3545;") # Error color
             elif "thành công" in lower_msg or "hoàn tất" in lower_msg:
                 self.opt_status_label.setStyleSheet("color: #28a745;") # Success color
             else:
                  self.opt_status_label.setStyleSheet("color: #6c757d;") # Default info color
        else:
    ...
    def show_calendar_dialog_qt(self, target_line_edit: QLineEdit):
        ...
        # Determine min/max/current dates
        ...
        current_qdate = QDate.currentDate() # Default to today
        try:
            ...
            # Ensure parsed date is within valid range
            if min_qdate <= parsed_qdate <= max_qdate:
                current_qdate = parsed_qdate
            else: # If current text is out of range, default to max valid date
                 current_qdate = max_qdate
        except ValueError:
            # If current text is invalid, default to max valid date
            current_qdate = max_qdate
        ...
        # Basic styling for the calendar widget itself
    ...
    def _load_optimization_log(self):
        ...
        log_path = target_dir / "optimization_qt.log" # Use the Qt log file name
        self.current_optimization_log_path = log_path # Store for appending later

        self.opt_log_text.clear() # Clear previous content
        cursor = self.opt_log_text.textCursor()
            ...
            try:
                with open(log_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        # Basic tag detection (can be improved)
                        tag = "INFO"
...
# =============================================================================
# == SECTION 2: LotteryPredictionApp (PyQt5 Version)                       ==
# =============================================================================
...
class LotteryPredictionApp(QMainWindow):
    def __init__(self):
        ...
        # No ui_theme_file_path needed anymore

        # --- Configurations (Identical) ---
        self.config = configparser.ConfigParser(interpolation=None)
        # No ui_theme_config needed anymore

        # --- Data & State (Identical) ---
        self.results = []
        self.selected_date = None
        self.algorithms = {} # Stores UI widgets and state for main tab algos
        self.algorithm_instances = {} # Stores actual algo instances

        # **** START MODIFICATION: Initialize font variables with defaults ****
        self.font_family_base = 'Segoe UI' # Default fallback
        self.font_size_base = 10           # Default fallback
        # **** END MODIFICATION ****

        # --- Worker/Threading (Keep Queues, Add Timers) ---
        self.calculation_queue = queue.Queue() # For prediction worker
        self.perf_queue = queue.Queue()        # For performance worker
        ...
        # QTimers for polling queues safely from GUI thread
        self.prediction_timer = QTimer(self)
        self.prediction_timer.timeout.connect(self.check_predictions_completion_qt)
        self.prediction_timer_interval = 200 # ms

        self.performance_timer = QTimer(self)
        self.performance_timer.timeout.connect(self._check_perf_queue)
        self.performance_timer_interval = 200 # ms

        # --- Misc ---
        self.optimizer_app_instance = None
        self.available_fonts = sorted(QFontDatabase().families()) # Get available fonts early

        # --- Initial Setup ---
        self.create_directories()
        self._setup_validators() # Setup validators first

        # **** START MODIFICATION: Changed order ****
        self.load_config() # Tải config vào self.config và cập nhật biến instance font
        self._setup_global_font() # Áp dụng font toàn cục
        self.setup_main_ui_structure() # Tạo tất cả UI, bao gồm tab Cài đặt
        self.apply_stylesheet()
        self._apply_window_size_from_config()

        self._populate_settings_tab_ui() # <<<< GỌI HÀM NÀY Ở ĐÂY
        ...
        self.load_data() # Load data AFTER config (uses data path from config)
        self.load_algorithms() # Load algorithms AFTER config (uses config for states)

        # Initialize Optimizer within its Tab
        ...
        # Populate settings UI AFTER load_config
    ...
    def _setup_global_font(self):
        ...
        try:
             qfont = self.get_qfont("base") # Get QFont object
    ...
    def _setup_validators(self):
         ...
         self.weight_validator = QDoubleValidator()
         self.weight_validator.setNotation(QDoubleValidator.StandardNotation) # Avoid scientific notation
         # Allow locale-specific decimal separators if needed:
         # self.weight_validator.setLocale(QtCore.QLocale.system())
    ...
    def setup_main_ui_structure(self):
        ...
        main_layout.setContentsMargins(5, 5, 5, 5) # Padding around the tab widget

        self.tab_widget = QTabWidget()
        self.tab_widget.setObjectName("MainTabWidget") # For potential styling

        # Create tab container widgets
        self.main_tab_frame = QWidget()
        self.optimizer_tab_frame = QWidget() # OptimizerEmbedded will be placed here
        ...
        #self.tab_widget.addTab(self.info_tab_frame, " Thông Tin 📒")

        main_layout.addWidget(self.tab_widget)

        # Setup content for each tab (layouts will be set within these methods)
        self.setup_main_tab()
        # Optimizer tab setup is handled after __init__ when the OptimizerEmbedded instance is created
        ...
        self.status_bar_label = QLabel("Khởi tạo...") # Use a label within the status bar
        self.status_bar_label.setObjectName("StatusBarLabel") # For styling
        self.status_bar.addWidget(self.status_bar_label, 1) # Stretch factor 1
    ...
    def _log_actual_window_size(self):
        try:
            if self: # Check if window still exists
    ...
    # --- Style and Theme Management (Simplified PyQt5) ---
    ...
    def load_ui_theme_config(self):
        ...
        try:
            ...
            # Read font settings, use defaults as fallbacks
            ...
            self.set_default_theme_values() # Set instance variables to defaults
    ...
    def get_default_theme_settings(self) -> dict:
        ...
        return {
            'Fonts': {
                'family_base': 'Segoe UI', # Common default, adjust if needed
                'size_base': 10,           # PyQt default size is often smaller
            }
        }
    ...
    def save_ui_theme_config(self):
        ...
        try:
            ...
            # Reload internal config immediately (though restart is needed for visual change)
    ...
    def reset_ui_theme_config(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                # Reset internal variables and update UI display in settings
    ...
    def apply_stylesheet(self):
        ...
        try:
            ...
            COLOR_CARD_BG = '#F0F0F0' # Light grey for card background
            ...
            """ # End of the main stylesheet string
    ...
    def get_qfont(self, font_type: str) -> QFont:
        ...
        font = QFont(base_family, base_size) # Start with base
        ...
        if font_type == "bold":
            ...
            font.setPointSize(max(6, base_size - 2)) # Ensure minimum size
        elif font_type == "italic_small":
            font.setPointSize(max(6, base_size - 2))
            font.setItalic(True)
        elif font_type == "code":
            font.setFamily('Consolas') # Fixed family for code
            font.setPointSize(base_size) # Use base size
    ...
    def setup_main_tab(self):
        ...
        # --- Top Section (Data Info + Controls) ---
        ...
        # --- Controls Group (Date Selection & Prediction) ---
        ...
        self.date_calendar_button.setObjectName("CalendarButton") # SET OBJECT NAME
        # self.date_calendar_button.setFixedWidth(30) # Removed fixed width
        ...
        # --- Algorithm List and Performance Section (using QSplitter) ---
        bottom_splitter = QSplitter(Qt.Horizontal)
        main_tab_layout.addWidget(bottom_splitter, 1)

        # --- Left Frame (Algorithm List) ---
        ...
        self.perf_start_date_button.setObjectName("CalendarButton") # SET OBJECT NAME
        # self.perf_start_date_button.setFixedWidth(30) # Removed fixed width
        ...
        self.perf_end_date_button.setObjectName("CalendarButton") # SET OBJECT NAME
        # self.perf_end_date_button.setFixedWidth(30) # Removed fixed width
    ...
    # --- INSIDE class LotteryPredictionApp ---
    def setup_settings_tab(self):
        ...
        settings_group_layout.setContentsMargins(10, 15, 10, 10) # Top margin for title space
        settings_group_layout.setHorizontalSpacing(10)
        settings_group_layout.setVerticalSpacing(12) # Vertical spacing between rows
        ...
        settings_group_layout.addWidget(self.config_data_path_edit, 0, 1, 1, 2) # Span 2 columns
        browse_button = QPushButton("📂")
        browse_button.setFixedWidth(40) # Make browse button smaller
        ...
        settings_group_layout.addWidget(self.config_sync_url_edit, 1, 1, 1, 3) # Span 3 columns
        ...
        settings_group_layout.addWidget(size_frame, 2, 1, 1, 3) # Span 3 columns

        # Row 3: Font Settings (Requires Restart)
        ...
        # **** START MODIFICATION: Remove stretch factor ****
        # font_layout.addWidget(self.theme_font_family_base_combo, 1) # Remove stretch factor 1
        font_layout.addWidget(self.theme_font_family_base_combo, 0) # Add with stretch factor 0 (takes minimum space)
        # Optionally, set a maximum width if needed:
        # self.theme_font_family_base_combo.setMaximumWidth(250) # Example maximum width
        # **** END MODIFICATION ****
        ...
        # **** ADD STRETCH HERE to push font controls left ****
        ...
        settings_group_layout.addWidget(separator, 4, 0, 1, 4) # Span all columns

        # Row 5: Config File Management Label
        settings_group_layout.addWidget(QLabel("⚙️ Quản lý file cấu hình khác:"), 5, 0, Qt.AlignLeft)

        # Row 6: Config File List
        self.config_listwidget = QListWidget()
        self.config_listwidget.setFixedHeight(80) # Adjusted height
        ...
        settings_group_layout.addWidget(self.config_listwidget, 6, 0, 1, 4) # Span all columns
        self.update_config_list() # Initial population of the list

        # --- Add the main group to the tab layout ---
        ...
        save_config_button.setObjectName("SettingsButton") # Use smaller button style
        ...
        save_new_cfg_button.setObjectName("SettingsButton") # Use smaller button style
        ...
        load_cfg_button.setObjectName("SettingsButton") # Use smaller button style
        ...
        reset_cfg_button.setObjectName("DangerButton") # Keep Danger style, size adjusted in stylesheet
        ...
        # Optional: Add stretch to push buttons left/right or center them
        # **** THÊM HOẶC BỎ COMMENT DÒNG NÀY ****
        button_layout.addStretch(1) # Đẩy các nút về bên trái

        settings_tab_layout.addWidget(button_frame)
        #settings_tab_layout.addStretch(1) # Đảm bảo dòng này vẫn được comment hoặc xóa
        info_groupbox = QGroupBox("") # Bạn có thể thêm tiêu đề nếu muốn, ví dụ: "Thông tin"

        # 2. Tạo QVBoxLayout với info_groupbox làm parent
        info_group_layout = QVBoxLayout(info_groupbox)
        info_group_layout.setContentsMargins(10, 15, 10, 10)
        info_group_layout.setSpacing(8) # Khoảng cách giữa các dòng thông tin

        # 3. Thêm các QLabel và căn giữa chúng
        # Version and Date
        ...
        version_label.setAlignment(Qt.AlignCenter) # Căn giữa
        info_group_layout.addWidget(version_label)

        # Libraries Header
        libs_label = QLabel("<b>Thư viện sử dụng:</b>")
        libs_label.setAlignment(Qt.AlignCenter) # Căn giữa
        ...
        libs_val_label.setStyleSheet("color: #17a2b8;") # Màu xanh info
        libs_val_label.setAlignment(Qt.AlignCenter) # Căn giữa
        info_group_layout.addWidget(libs_val_label)

        # System Info Header
        sys_info_title_label = QLabel("<b>Thư mục gốc:</b>")
        sys_info_title_label.setAlignment(Qt.AlignCenter) # Căn giữa
        info_group_layout.addWidget(sys_info_title_label)

        # System Info Details
        sys_info = f"{self.base_dir}"
        sys_info_label = QLabel(sys_info)
        sys_info_label.setTextFormat(Qt.RichText) # Cho phép dùng <br>
        sys_info_label.setStyleSheet("color: #17a2b8;") # Màu xanh info
        sys_info_label.setAlignment(Qt.AlignCenter) # Căn giữa
        info_group_layout.addWidget(sys_info_label)

        # 4. Đẩy nội dung lên trên trong GroupBox
        #info_group_layout.addStretch(1)

        # 5. Thêm GroupBox thông tin vào layout chính của tab
        settings_tab_layout.addWidget(info_groupbox)

        #settings_tab_layout.addStretch(1) # Push ALL content (Settings + Info) to the top

        # Populate the UI fields AFTER they are created using the helper function
        # Note: The actual values are loaded into self.config earlier in __init__ or other methods
    ...
    def _populate_settings_tab_ui(self):
        ...
        try:
            ...
            default_width = 1200
            default_height = 800
            # Instance variables for font should be set correctly by load_config already
            default_font_family = self.font_family_base
            default_font_size = self.font_size_base

            # Data File Path
            if hasattr(self, 'config_data_path_edit'):
                # Luôn đọc trực tiếp từ self.config khi cập nhật UI này
            ...
            if hasattr(self, 'config_sync_url_edit'):
                ...
                self.config_sync_url_edit.setText(sync_url) # <--- Dòng quan trọng để cập nhật ô URL
            ...
            if hasattr(self, 'window_width_edit'):
                ...
                self.window_width_edit.setText(width_str) # SetText với string
            else:
            ...
            if hasattr(self, 'window_height_edit'):
                ...
                self.window_height_edit.setText(height_str) # SetText với string
            else:
            ...
            # Font Settings
            if hasattr(self, 'theme_font_family_base_combo'):
                # Sử dụng biến instance đã được xác thực trong load_config
                font_family = self.font_family_base
                # Tìm index của font trong combobox để đặt giá trị chính xác
                ...
                else: # Fallback nếu font không có trong combobox (dù đã kiểm tra trước)
            ...
            # Optionally show a message if population fails critically
            QMessageBox.warning(self, "Lỗi UI", f"Không thể cập nhật đầy đủ giao diện cài đặt:\n{e}")
            
    # --- (Keep _setup_performance_text_formats method as is) ---
    def _setup_performance_text_formats(self):
        ...
        def create_format(font, color_hex, weight=None, underline=False):
            ...
            fmt.setFont(font)
            fmt.setForeground(QColor(color_hex))
            if weight: fmt.setFontWeight(weight) # Use QFont constants if needed
            fmt.setFontUnderline(underline)
            return fmt

        # Section Header (like === Title ===)
        fmt_header = create_format(code_bold_underline_font, '#0056b3') # Use bold underline font
        fmt_header.setFontPointSize(code_font.pointSize() + 1) # Slightly larger
        self.perf_text_formats["section_header"] = fmt_header

        # Error text
        self.perf_text_formats["error"] = create_format(code_font, '#dc3545') # Red

        # Normal text (can just use default or define explicitly)
        self.perf_text_formats["normal"] = create_format(code_font, '#212529')

    # --- (Keep setup_settings_tab method as is) ---
    ...
    def load_config(self, config_filename="settings.ini"):
        ...
        # Ensure instance config parser is created/reset for this load operation
        self.config = configparser.ConfigParser(interpolation=None)
        # Flag to track if config needs saving after loading (due to defaults/validation)
        config_needs_saving = False

        try:
            if config_path.exists():
                # Read the file into the config object
                read_files = self.config.read(config_path, encoding='utf-8')
                if not read_files: # Check if read was successful (e.g., permissions issue)
                     ...
                     self.set_default_config() # Load defaults into self.config
                ...
                # Set default values in the self.config object
                self.set_default_config()
                config_needs_saving = True # Mark for saving as defaults were used

            # --- Update INSTANCE VARIABLES and UI elements NOT on Settings tab ---

            # DATA section
            default_data_path = str(self.data_dir / "xsmb-2-digits.json")
            default_sync_url = "https://raw.githubusercontent.com/khiemdoan/vietnam-lottery-xsmb-analysis/main/data/xsmb-2-digits.json"
            # Ensure section exists before getting options
            ...
            # Get data path and sync url (these values are needed internally and for other UI parts)
            ...
            # Update UI elements OUTSIDE the Settings tab if they exist
            if hasattr(self, 'sync_url_input'): # Main tab sync input
                self.sync_url_input.setText(sync_url_from_config)
            if hasattr(self, 'data_file_path_label'): # Main tab data path label
                self.data_file_path_label.setText(data_file_from_config)
                self.data_file_path_label.setToolTip(data_file_from_config)

            # UI Section (Size and Font) - Update instance variables
            default_width, default_height = 1200, 800
            default_font_family = 'Segoe UI' # Default font if not specified or invalid
            ...
            # Size - Read values but don't update Settings UI widgets here
            try:
                # Store loaded width/height for later use by _apply_window_size_from_config
                # Getint will raise ValueError if not integer, caught below
                ...
                # Correct config object if getint's fallback was used due to non-integer value
                ...
                # Catch if value exists but isn't an integer
                ...
                # Update config object with defaults if read failed
                self.config.set('UI', 'width', str(default_width))
                self.config.set('UI', 'height', str(default_height))
                config_needs_saving = True

            # Font Settings - Update INSTANCE VARIABLES ONLY
            try:
                # Load font family from config
                loaded_font_family = self.config.get('UI', 'font_family_base', fallback=default_font_family)
                # Validate font family against available system fonts
                if not self.available_fonts: # Safety check
                    ...
                    self.font_family_base = default_font_family
                    # Update the config object with the valid fallback font
                    self.config.set('UI', 'font_family_base', self.font_family_base)
                    config_needs_saving = True # Mark for saving because config was corrected
                else:
                    self.font_family_base = loaded_font_family # Use the valid font from config

                # Load font size from config
                original_size_str = self.config.get('UI', 'font_size_base', fallback=str(default_font_size))
                # Use getint which handles fallback and raises ValueError if not integer
                loaded_font_size = self.config.getint('UI', 'font_size_base', fallback=default_font_size)
                # Apply range validation (e.g., 8pt to 24pt)
                validated_font_size = max(8, min(24, loaded_font_size))
                self.font_size_base = validated_font_size
                # Update config object if validation changed the size
                ...
                # Catch if font size exists but isn't integer or other config errors
            ...
            # REMOVED UI updates for Settings Tab widgets here
            # REMOVED calls to _setup_global_font() and populate_theme_settings_ui() here

            # Save config if defaults were added/modified during loading (only for settings.ini)
            if is_main_settings and config_needs_saving:
                 main_logger.info("Config needed saving after loading defaults/validation.")
                 self._save_default_config_if_needed(self.settings_file_path) # Save current state of self.config

        except configparser.Error as e:
            # Catch general ConfigParser errors (e.g., parsing errors like missing sections handled above, but others possible)
            main_logger.error(f"Error parsing config file {config_path}: {e}. Setting defaults.")
            self.set_default_config() # Sets self.config to defaults
            self._apply_default_config_to_vars() # Update instance vars from default config
            # Attempt to save the defaults if it was the main settings file
            if is_main_settings:
    ...
    # Remember to include the helper method within the class:

    # You also need the helper method mentioned before:
    def _save_default_config_if_needed(self, config_path):
        """Saves the current self.config (assumed defaults) to the specified path."""
        try:
            # Ensure sections exist before writing (redundant if set_default_config ensures this)
    ...
    # --- (Keep apply_algorithm_config_states as corrected previously) ---
    def apply_algorithm_config_states(self):
        ...
        for algo_name, algo_data in self.algorithms.items():
            # Get references to the UI widgets stored in algo_data
            ...
            config_section = algo_name # Section name matches algo display name

            # **** START MODIFICATION: Use try...except instead of if/else ****
            try:
                # Directly attempt to add the section.
                # If it succeeds, the section didn't exist, so set defaults.
                ...
                config_changed = True # Mark that config was changed

            except configparser.DuplicateSectionError:
                # If add_section fails with DuplicateSectionError, the section already existed.
                # Proceed to load the existing values from the config.
                ...
                try:
                    ...
                    # Validate weight value before setting UI
                    if not self._is_valid_float_str(weight_value_str):
                        ...
                        # Optionally update the config object with the corrected value
                        # self.config.set(config_section, 'weight_value', weight_value_str)
                        # config_changed = True

                    # Update UI widgets from config values
                    ...
                    # Handle potential errors reading options within the existing section
                    main_logger.error(f"Error reading options from existing section '{config_section}': {e}. Setting defaults for UI.", exc_info=True)
                    # Set UI to defaults as a fallback
                    chk_enable.setChecked(True)
                    chk_weight.setChecked(False)
                    weight_entry.setText("1.0")
            # **** END MODIFICATION ****

            # Update enable state of weight controls based on main checkbox state
            # This should run regardless of whether the section was new or existing
            self._update_dependent_weight_widgets(algo_name)

        # Save the main config file if defaults were added or potentially corrected
        if config_changed:
            main_logger.info("Saving config after potentially adding/correcting algorithm sections.")
            try:
                 self.save_config("settings.ini") # Save changes to the main settings file
            except Exception as e:
                 main_logger.error(f"Failed to save config after applying defaults/corrections: {e}", exc_info=True)

    # --- (Keep _apply_default_config_to_vars as corrected previously) ---
    ...
    def set_default_config(self):
        ...
        # **** ADD FONT DEFAULTS HERE ****
        self.config['UI'] = {
            'width': '1200',
            'height': '800',
            'font_family_base': 'Segoe UI', # Default font family
            'font_size_base': '10'          # Default font size (as string)
        }
        # Algorithm sections will be added dynamically if needed

    # --- (Keep save_config_from_settings_ui as is) ---
    def save_config_from_settings_ui(self):
         ...
         try:
            # Update self.config object from UI fields
            if not self.config.has_section('DATA'): self.config.add_section('DATA')
            ...
            # Basic validation (should be covered by validators, but double-check)
            ...
            # No min size check here, just save what's entered (or default)
            self.config.set('UI', 'width', str(w))
            self.config.set('UI', 'height', str(h))

            # Algorithm states are saved via toggle functions, BUT ensure config reflects UI *before* saving
            ...
            # Write the config object to the main settings file
            ...
            self._apply_window_size_from_config() # Apply the new size
    ...
    # --- (Keep save_config method as corrected previously) ---
    def save_config(self, config_filename="settings.ini"):
        """Saves the current self.config object to the specified file."""
        # This method assumes self.config accurately reflects the state to be saved.
        main_logger.debug(f"Saving config object to file: {config_filename}...")
        save_path = self.config_dir / config_filename
        try:
            # Ensure DATA and UI sections exist before writing
            if not self.config.has_section('DATA'): self.config.add_section('DATA')
            if not self.config.has_section('UI'): self.config.add_section('UI')
            # Ensure basic UI options exist (with fallbacks to prevent errors if config is minimal)
            ...
            # Algorithm sections should already be updated in self.config by UI interactions

            # Write the config object
            with open(save_path, 'w', encoding='utf-8') as configfile:
                self.config.write(configfile)

            # If saving the main settings file, apply relevant immediate changes
            if config_filename == "settings.ini":
                # Update sync url in main tab if it exists
                if hasattr(self, 'sync_url_input'):
                     # Use fallback for get in case DATA/sync_url somehow disappeared despite checks
                     self.sync_url_input.setText(self.config.get('DATA','sync_url', fallback=''))
                # Apply window size
                self._apply_window_size_from_config()

            main_logger.info(f"Configuration saved to: {save_path}")
            # Let the calling function handle status/messagebox
    ...
    def save_current_settings_to_main_config(self):
        ...
        try:
            ...
            # Algorithm states are assumed to be updated in self.config via their toggles

            # --- Write the updated config object to settings.ini ---
            self.save_config("settings.ini") # Use the generic save function

            # --- Apply changes that can take effect immediately ---
            if hasattr(self, 'sync_url_input'):
                self.sync_url_input.setText(self.config_sync_url_edit.text())
            self._apply_window_size_from_config() # Apply the new size
            # Update internal font variables (though UI needs restart for full effect)
    ...
    def save_config_dialog(self):
        ...
        try:
            ...
            filename, _ = QFileDialog.getSaveFileName(
                self, # Parent window
                "Lưu Cấu Hình App Hiện Tại Thành File Mới", # Updated title
                str(self.config_dir / default_name),
                "Config files (*.ini);;All files (*.*)"
            )
            if filename:
                new_filename = Path(filename).name
                # Prevent overwriting key system files
                ...
                # Create a temporary config object reflecting the *current* UI state
                temp_config_obj = configparser.ConfigParser(interpolation=None)

                # Populate from current UI state
                ...
                # Write the temporary config object to the chosen file
                with open(filename, 'w', encoding='utf-8') as configfile:
                    temp_config_obj.write(configfile)

                self.update_config_list() # Refresh the listbox
    ...
    def load_config_dialog(self):
        ...
        try:
            filename, _ = QFileDialog.getOpenFileName(
                ...
                str(self.config_dir), # Initial directory
                "Config files (*.ini);;All files (*.*)"
            )
            if filename:
                self.load_config_from_file(Path(filename).name) # Load using the filename
        except Exception as e:
             QMessageBox.critical(self, "Lỗi", f"Đã xảy ra lỗi khi chọn file:\n{e}")

    # --- (Keep load_selected_config_qt as is) ---
    def load_selected_config_qt(self, item: QListWidgetItem):
        ...
        """
        # Kiểm tra xem item có hợp lệ không (được cung cấp bởi signal)
        if item:
            # Lấy tên file (dưới dạng string) từ mục được double-click
            filename = item.text()
            main_logger.info(f"Config file selected from list (double-click): {filename}")

            # Gọi hàm chính để tải và áp dụng cấu hình.
            # Hàm này đã bao gồm xử lý lỗi và thông báo cho người dùng.
            try:
                self.load_config_from_file(filename)
            except Exception as e:
                # Mặc dù load_config_from_file đã có try-except, thêm một lớp ở đây
                # để bắt các lỗi không mong muốn ngay tại điểm gọi này.
            ...
            # Trường hợp này không nên xảy ra nếu chỉ được kích hoạt bởi signal double-click,
            # nhưng thêm vào để phòng ngừa.
            main_logger.warning("load_selected_config_qt called with no valid item.")

    # --- (Keep update_config_list as is) ---
    def update_config_list(self):
        """Updates the QListWidget with available .ini configuration files."""
        try:
            if not hasattr(self, 'config_listwidget'): return
            self.config_listwidget.clear() # Clear existing items
    ...
    def reset_config(self):
        ...
        reply = QMessageBox.question(
            self, # Parent
            "Xác nhận Khôi Phục Mặc Định", # Updated title
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                if config_path.exists():
                    try:
                        config_path.unlink() # Delete the existing file
                        main_logger.info(f"Deleted existing settings file: {config_path}")
                    except OSError as e:
                        # **** CORRECTED INDENTATION HERE ****
                        # This block now correctly contains indented code
                        QMessageBox.critical(self, "Lỗi Xóa", f"Không thể xóa file '{config_path.name}':\n{e}")
                        # Optionally, consider if you should proceed or return here
                        # return # Example: Stop reset if deletion fails
                        main_logger.error(f"Proceeding with reset despite failing to delete {config_path.name}.")
                        # **** END CORRECTION ****

                # --- Start main reset logic ---
                # 1. Set self.config to default values
                self.set_default_config()

                # 2. Save the defaults to settings.ini (using the helper ensures sections exist)
                self._save_default_config_if_needed(self.settings_file_path)

                # 3. Update instance variables (this might be redundant if set_default_config already did it, but safe)
                self._apply_default_config_to_vars()

                # 4. Populate the Settings tab UI with the new default values
                self._populate_settings_tab_ui()

                # 5. Apply immediate changes (window size, update non-settings UI)
                self._apply_window_size_from_config() # Apply default window size
                # Update Main tab sync URL input
                if hasattr(self, 'sync_url_input'):
                     sync_url = self.config.get('DATA', 'sync_url', fallback="")
                     self.sync_url_input.setText(sync_url)

                # 6. Reload data and algorithms based on the new default config
                self.reload_algorithms() # Reloads algos and applies default states from the now-default self.config
                self.load_data()         # Reload data based on default path in self.config

                # 7. Sync optimizer if it exists
                ...
                # --- End main reset logic ---

            except Exception as e:
                # Catch errors during the main reset process (after potential deletion attempt)
    ...
    def load_config_from_file(self, filename):
        ...
        try:
            ...
            # 1. Load the new config into self.config
            self.load_config(filename) # Use the main load_config

            # 2. Populate the Settings tab UI
            self._populate_settings_tab_ui()

            # 3. Apply settings requiring explicit action
            self._apply_window_size_from_config()

            # 4. Reload algorithms and data
    ...
    # --- Data Loading and Handling Methods (PyQt5 Adaptation) ---
    # --- (Keep load_data method as corrected previously) ---
    def load_data(self):
        """Loads lottery result data based on the path in the current config."""
        self.results = []
        main_logger.info("Loading lottery data (PyQt5)...")
        try:
            # Ensure DATA section and data_file option exist in config
            if not self.config.has_section('DATA') or not self.config.has_option('DATA', 'data_file'):
                ...
                self.set_default_config()
                # Update UI fields in settings tab after setting defaults
                self._apply_default_config_to_vars()
                # Save the updated config (containing defaults)
                self.save_config() # Save to settings.ini

            # Get data file path from config
            # Use fallback with keyword argument
            data_file_str = self.config.get('DATA', 'data_file', fallback="")
            if not data_file_str:
                # If still empty after potential default setting, use hardcoded default
                ...
                # Update config object and UI field
                ...
                # Save the config if we had to add the fallback path
                self.save_config()

            data_file_path = Path(data_file_str)

            # Update the label in the Main tab
            if hasattr(self, 'data_file_path_label'):
                self.data_file_path_label.setText(str(data_file_path))
                self.data_file_path_label.setToolTip(str(data_file_path)) # Add tooltip
            ...
            if not data_file_path.exists():
                ...
                # Try creating sample if it's the expected default file
                if data_file_path == self.data_dir / "xsmb-2-digits.json":
                    self.create_directories() # Attempts to create sample
                    if data_file_path.exists():
                        main_logger.info("Sample data created. Reloading data...")
                        QTimer.singleShot(100, self.load_data) # Schedule reload shortly after
                        return
                    else:
                    ...
                    return # Stop loading if specific file not found

            # --- Read and Process Data (Identical Logic) ---
            ...
            for item in data_list_to_process:
                 ...
                 date_str_raw = item.get("date");
                 if not date_str_raw: continue
                 date_str_cleaned = str(date_str_raw).split('T')[0] # Handle ISO format dates
                 try: date_obj = datetime.datetime.strptime(date_str_cleaned, '%Y-%m-%d').date();
                 except ValueError: continue # Skip invalid date formats
                 if date_obj in unique_dates: continue # Skip duplicate dates
                 result_data = item.get('result');
                 # Handle cases where 'result' key might be missing but data is at top level
                 if result_data is None: result_data = {k: v for k, v in item.items() if k != 'date'}
                 if not result_data: continue # Skip if no result data found
                 results_temp.append({'date': date_obj, 'result': result_data}); unique_dates.add(date_obj); processed_count += 1
            # --- End Data Processing ---

            # --- Update UI based on processed data ---
            if results_temp:
                results_temp.sort(key=lambda x: x['date'])
                self.results = results_temp # Store the final sorted list
                ...
                # Update selected date (main prediction date)
                ...
                if current_selection_str:
                    try:
                        ...
                        if start_date <= current_selection_date <= end_date:
                            self.selected_date = current_selection_date # Keep current valid selection
                            needs_update = False
                    except ValueError: pass # Invalid format, will update

                if needs_update:
                    self.selected_date = end_date # Default to latest date
                    if hasattr(self, 'selected_date_edit'): self.selected_date_edit.setText(end_ui)

                # Update default performance dates
            ...
            else: # No valid results processed
    ...
    # --- (Keep _update_default_perf_dates as corrected previously) ---
    def _update_default_perf_dates(self, data_start_date, data_end_date):
        ...
        # Check if current start date is valid and within new range
            ...
            if current_perf_start_str:
                try:
                    ...
                    if data_start_date <= current_perf_start <= data_end_date:
                        update_start = False # Keep existing valid date
                except ValueError: pass # Invalid format, will update

        # Check if current end date is valid and within new range
            ...
            if current_perf_end_str:
                try:
                    ...
                    if data_start_date <= current_perf_end <= data_end_date:
                        update_end = False # Keep existing valid date
                except ValueError: pass # Invalid format, will update
    ...
    # --- (Keep change_data_path as corrected previously) ---
    def change_data_path(self):
        ...
        try:
            ...
            if filename:
                ...
                # Update settings UI field directly
                if hasattr(self, 'config_data_path_edit'):
                     self.config_data_path_edit.setText(str(new_path))

                # Save the config change
                self.save_config() # Saves to settings.ini

                # Reload data and algorithms
                self.load_data()
                self.reload_algorithms() # Reloads algorithms, which also applies config states
    ...
    # --- (Keep browse_data_file_settings as is) ---
    def browse_data_file_settings(self):
        """Opens file dialog specifically for the data path in the Settings tab."""
        try:
            current_path_str = self.config_data_path_edit.text() # Get from settings UI
            initial_dir = str(self.data_dir)
            if current_path_str:
                parent_dir = Path(current_path_str).parent
                if parent_dir.is_dir():
            ...
            if filename:
                # Update only the Settings UI field
                self.config_data_path_edit.setText(filename)
                # User needs to click "Save Config" to apply this change
        except Exception as e:
             QMessageBox.critical(self, "Lỗi", f"Lỗi duyệt file:\n{e}")

    # --- (Keep sync_data as corrected previously) ---
    def sync_data(self):
        """Downloads data from the sync URL and replaces the current data file."""
        url_to_sync = self.sync_url_input.text().strip() # Get from Main tab input
        if not url_to_sync:
        ...
        try:
            ...
            # Validate downloaded JSON content *before* overwriting
            try:
                 downloaded_data = response.json()
                 # Basic format check (adjust as needed for your specific format)
            ...
            # Write downloaded content to target file
            try:
                with open(target_file, 'wb') as f: # Write in binary mode
                ...
                if backed_up_successfully:
                    self._restore_backup(backup_file, target_file) # Attempt restore
                self.update_status("Đồng bộ thất bại: lỗi ghi file.")
                return

            # Reload data and algorithms after successful sync
            self.load_data()
            self.reload_algorithms() # Reload algorithms and apply states

            # Sync optimizer if it exists
            if self.optimizer_app_instance:
            ...
            if backed_up_successfully: self._restore_backup(backup_file, target_file) # Attempt restore
        except Exception as e:
    ...
    # --- Algorithm Loading (PyQt5 Adaptation) ---
    ...
    def load_algorithms(self):
        ...
        # Clear existing algorithm UI elements from the layout
        ...
        self.algorithms.clear() # Clears UI widget refs and state
        self.algorithm_instances.clear() # Clears actual algorithm instances
        ...
        # Create sample algorithms if directory is empty
        if not algorithm_files_to_load:
             self.create_sample_algorithms()
             # Rescan after creating samples
             algorithm_files_to_load = [
                f for f in self.algorithms_dir.glob('*.py')
                if f.is_file() and f.name not in ["__init__.py", "base.py"]
            ]
        ...
        # Prepare data for instances (use deep copy)
        results_copy_for_instances = copy.deepcopy(self.results) if self.results else []
        cache_dir_for_instances = self.calculate_dir

        # --- Load Each Algorithm File ---
        initial_algo_label = None # Placeholder for "no algorithms" message
        if not algorithm_files_to_load:
        ...
        # Show warnings/errors if necessary
        if count_failed > 0 and count_success > 0:
        ...
        elif count_success == 0 and algorithm_files_to_load: # Files exist but no valid classes found
            ...
            if hasattr(self, 'algo_list_layout') and self.algo_list_layout.count() == 0:
                 if initial_algo_label is None: # Ensure label is created if needed
        ...
        # Apply loaded config states (enabled, weights) to the UI
        self.apply_algorithm_config_states()
    ...
    def load_algorithm_from_file(self, algo_file_path: Path, data_results_list: list, cache_dir: Path) -> bool:
        ...
        success = False
        module_obj = None
        display_name = f"Unknown ({algo_file_path.name})" # Default display name

        try:
            # --- Module Loading/Reloading (Identical Logic) ---
            if module_name in sys.modules:
                ...
                try: module_obj = reload(sys.modules[module_name])
                    ...
                    module_obj = None # Force re-import
            else:
            ...
            if module_obj is None:
                 ...
                 if spec and spec.loader:
                     module_obj = util.module_from_spec(spec)
                     sys.modules[module_name] = module_obj # Add to sys.modules *before* exec
                     spec.loader.exec_module(module_obj)
                 else:
                     main_logger.error(f"Could not create module spec/loader for {algo_file_path.name}")
                     return False # Cannot proceed without spec/loader
            if not module_obj:
                main_logger.error(f"Module object is None after loading attempt for {algo_file_path.name}")
                return False
            # --- End Module Loading ---

            # --- Find Algorithm Class (Identical Logic) ---
            found_class = None
            found_class_name = None
            for name, obj in inspect.getmembers(module_obj):
                # Check if it's a class defined in *this* module, is a subclass of BaseAlgorithm, but not BaseAlgorithm itself
                if inspect.isclass(obj) and issubclass(obj, BaseAlgorithm) and obj is not BaseAlgorithm and obj.__module__ == module_name:
                    found_class = obj
                    found_class_name = name
                    display_name = f"{name} ({algo_file_path.name})" # Update display name
                    main_logger.debug(f"Found valid algorithm class '{name}' in {algo_file_path.name}")
                    break # Found the first valid class, stop searching
            # --- End Class Finding ---
            ...
            if found_class:
                try:
                    ...
                    # Clean up if instantiation failed
                    if display_name in self.algorithm_instances: del self.algorithm_instances[display_name]
                    if display_name in self.algorithms: del self.algorithms[display_name] # Remove UI data as well
                    success = False
            else:
                # No valid class found in the file
        ...
        # Cleanup module reference if loading failed to prevent potential issues
        if not success and module_name in sys.modules:
             try:
                 # Only delete if we actually loaded it in this function call
                 if module_name in sys.modules and sys.modules[module_name] == module_obj:
                     del sys.modules[module_name]
             except KeyError: pass
             except NameError: pass # module_obj might not be defined if spec failed
    ...
    def create_algorithm_ui_qt(self, algo_name, algo_config, algo_filename):
        ...
        try:
            ...
            # --- Main Frame (Card) ---
            algo_frame = QFrame()
            algo_frame.setObjectName("CardFrame") # Set object name for styling
            # Apply 3D effect directly
            algo_frame.setFrameShape(QFrame.StyledPanel) # Shape needed for shadow
            algo_frame.setFrameShadow(QFrame.Raised)    # Make it look raised
            algo_frame.setLineWidth(1)                  # Define border width for shadow

            algo_layout = QVBoxLayout(algo_frame)
            algo_layout.setSpacing(6) # Moderate spacing inside card
            algo_layout.setContentsMargins(10, 8, 10, 8) # Padding inside card
            ...
            name_file_label.setStyleSheet("padding-bottom: 2px; color: #0056b3;") # Add space below, darker color
            name_file_label.setToolTip(f"Thuật toán: {class_name_only}\nFile: {algo_filename}")
            algo_layout.addWidget(name_file_label)

            # --- Middle: Description (Moved here) ---
            description = algo_config.get("description", "Không có mô tả.")
            desc_label = QLabel(description)
            desc_label.setWordWrap(True)
            desc_label.setFont(self.get_qfont("small")) # Use smaller font for description
            desc_label.setStyleSheet("color: #5a5a5a; padding-bottom: 6px;") # Slightly darker grey, more space below
            desc_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
            desc_label.setToolTip(description)
            algo_layout.addWidget(desc_label) # Add directly to the main vertical layout

            # --- Bottom Row: Controls (Enable + Weight) ---
            control_row_widget = QWidget() # Use one horizontal layout for all controls
            control_row_layout = QHBoxLayout(control_row_widget)
            control_row_layout.setContentsMargins(0, 0, 0, 0) # No extra margins for this row
            control_row_layout.setSpacing(10) # Spacing between control groups
            ...
            control_row_layout.addSpacing(5) # Add a small space

            # Weight Controls (Checkbox + Entry)
            chk_weight = QCheckBox("Hệ số:") # Shortened label
            chk_weight.setToolTip("Áp dụng hệ số nhân cho điểm số của thuật toán này khi kết hợp.")
            weight_entry = QLineEdit("1.0")
            weight_entry.setFixedWidth(70) # Slightly smaller width
            ...
            # Initial state for weight controls (disabled)
            ...
            control_row_layout.addStretch(1) # Push controls to the left

            algo_layout.addWidget(control_row_widget) # Add the control row widget
            ...
            # --- Add card to the main list layout ---
            self.algo_list_layout.addWidget(algo_frame)
    ...
    def toggle_algorithm(self, algo_name, chk_enable_widget):
        ...
        try:
            # Update dependent weight widgets first
            self._update_dependent_weight_widgets(algo_name)
            ...
            # --- Save Config ---
            try:
                self.save_config("settings.ini") # Save immediately to main config
            except Exception as save_err:
                 main_logger.error(f"Failed to save config after toggling {algo_name}: {save_err}", exc_info=True)
                 raise save_err # Re-raise to be caught by the outer try-except block

            self.update_status(f"Đã {state_text.lower()} thuật toán: {algo_name.split(' (')[0]}")

        except Exception as e: # Catch errors from widget access OR the save operation
            main_logger.error(f"Error toggling algorithm '{algo_name}': {e}", exc_info=True) # Log the error with traceback
            # Attempt to revert checkbox state on error
            try:
                # Disconnect the signal temporarily to prevent recursion during revert
            ...
            try:
                ...
                # Manually re-trigger dependent widget state updates after revert
                self._update_dependent_weight_widgets(algo_name)
                 ...
                 except Exception: pass # Avoid connecting multiple times if already connected somehow
    ...
    def toggle_algorithm_weight(self, algo_name, chk_weight_widget, weight_entry_widget):
        ...
        # Ensure the main algorithm is enabled first
        chk_enable = algo_data.get('chk_enable')
        if not chk_enable or not chk_enable.isChecked():
            # If main is disabled, force weight off and disable entry
        ...
        try:
            # Enable/disable the weight entry based on the checkbox
            weight_entry_widget.setEnabled(new_weight_state)
            if new_weight_state:
                weight_entry_widget.setFocus() # Focus on entry when enabled

            # --- Update and Save Config ---
            config_section = algo_name
            if not self.config.has_section(config_section): self.config.add_section(config_section)
            self.config.set(config_section, 'weight_enabled', str(new_weight_state))
            # Also save the current weight value when toggling, ensure it's valid
            ...
            if weight_entry_widget.text() != weight_to_save: weight_entry_widget.setText(weight_to_save) # Correct UI if needed
            self.config.set(config_section, 'weight_value', weight_to_save)
            try:
                self.save_config("settings.ini") # Save changes
            except Exception as save_err:
                 main_logger.error(f"Failed to save config after toggling weight for {algo_name}: {save_err}", exc_info=True)
                 raise save_err # Re-raise
            # --- End Config Update ---
            ...
            # Attempt to revert checkbox state and entry enable state on error
            try:
                # Disconnect toggle signal temporarily
                chk_weight_widget.toggled.disconnect()
            except TypeError: pass
            try:
                chk_weight_widget.setChecked(not new_weight_state)
                weight_entry_widget.setEnabled(not new_weight_state) # Revert entry state too
            except Exception as revert_err:
                 main_logger.error(f"Error reverting weight checkbox state for '{algo_name}': {revert_err}")
            finally:
                 # Reconnect toggle signal
                 try:
    ...
    def save_algorithm_weight_from_ui(self, algo_name, weight_entry_widget):
        ...
        if not weight_entry_widget.hasAcceptableInput():
            # Optionally provide visual feedback (e.g., red border)
            # weight_entry_widget.setStyleSheet("border: 1px solid red;")
            return

        # Clear potential error styling
        # weight_entry_widget.setStyleSheet("") # Or reset to default
        ...
        # Only save if the weight feature is actually enabled in the UI
        if chk_enable and chk_enable.isChecked() and chk_weight and chk_weight.isChecked():
             try:
                 ...
                 # Minimal validation here as validator should handle it
                 ...
                 # Only save if the value actually changed in the config
                 ...
                 if weight_value != current_config_value:
                      ...
                      try:
                          self.save_config("settings.ini") # Save the change
                          main_logger.debug(f"Saved weight '{weight_value}' for algorithm '{algo_name}'")
                           ...
                           # Optionally show an error message here, but might be too frequent
                           # QMessageBox.warning(self, "Lỗi Lưu", f"Lỗi lưu hệ số cho {algo_name}: {save_err}")
    ...
    def reload_algorithms(self):
        ...
        # Reload Main tab algorithms
        self.load_algorithms() # This handles UI clearing, loading, instance creation, and applying config

        # Reload Optimizer tab algorithms
        if self.optimizer_app_instance:
    ...
    def create_sample_algorithms(self):
        ...
        try:
            ...
            # Sample definitions (identical to Tkinter version)
            samples = {
    ...
    def show_calendar_dialog_qt(self, target_line_edit: QLineEdit, callback=None):
        ...
        # Keep the modification from the previous step to widen the popup
        if not self.results:
            QMessageBox.information(self, "Thiếu Dữ Liệu", "Chưa có dữ liệu kết quả để chọn ngày.")
            return

        # Determine valid date range and current selection
        ...
        current_text = target_line_edit.text()
        current_qdate = QDate.currentDate() # Fallback
        try:
            parsed_dt = datetime.datetime.strptime(current_text, '%d/%m/%Y').date()
            parsed_qdate = QDate(parsed_dt.year, parsed_dt.month, parsed_dt.day)
            if min_qdate <= parsed_qdate <= max_qdate:
        ...
        # --- Set Calendar/Dialog Width ---
        # Set minimum width for the calendar widget itself
        try:
            hint_width = calendar.sizeHint().width()
            # Set minimum width to 1.5x hint, but not less than 450px
            ...
            # Let the dialog resize based on the calendar's minimum width
            ...
            dialog.setMinimumWidth(500) # Fallback if setting calendar width fails
    ...
    # --- Prediction Process (PyQt5 Adaptation) ---
    ...
    def start_prediction_process(self):
        ...
        # Get actual results for the day *after* the prediction date (for comparison)
        ...
        self.calculation_queue = queue.Queue() # Reset queue
        self.calculation_threads = []

        # Clear cache directory (optional, but good practice)
        if self.calculate_dir.exists():
            main_logger.info(f"Clearing calculation cache: {self.calculate_dir}")
            try:
                # Keep this simple, log errors but don't block prediction
                for item in self.calculate_dir.iterdir():
                    try:
        ...
        try:
            if hasattr(self, 'predict_progress_frame'):
                ...
                if hasattr(self, 'predict_status_label'):
                     ...
                     self.predict_status_label.setObjectName("ProgressRunning") # Style if needed
        ...
        # Use a deep copy of history for threads to avoid race conditions if algos modify it
        ...
        for algo_name, instance in active_algorithm_instances.items():
            thread = threading.Thread(
                target=self.run_single_algorithm_prediction, # Keep using the same worker logic
    ...
    def run_single_algorithm_prediction(self, algo_name, algo_instance, date_to_predict, historical_results):
        ...
        # (Identical to Tkinter version - runs in separate thread)
        ...
        success = False
        try:
            # The core call to the algorithm's prediction logic
            scores = algo_instance.predict(date_to_predict, historical_results)

            # Basic validation of the returned scores
            if not isinstance(scores, dict):
                prediction_logger.error(f"[{thread_name}] Algorithm '{algo_name}' predict() method returned type {type(scores)} instead of dict.")
                scores = {} # Ensure it's a dict for downstream processing
                success = False
            else:
                # Optional: More detailed validation of keys/values
                invalid_items = []
                ...
                if invalid_items:
                    ...
                    # Filter out invalid items if necessary, or just log
            ...
            scores = {} # Return empty dict on error
            success = False
        finally:
            # --- Communicate result back to main thread via queue ---
            # Store the actual result (or empty dict on failure)
            with self._results_lock:
                self.intermediate_results[algo_name] = scores
            # Put a signal into the queue (algo_name on success, None on failure)
    ...
    def check_predictions_completion_qt(self):
        ...
        # Retrieve comparison data stored earlier
        ...
        if not all(hasattr(self, w) and getattr(self, w) for w in ['predict_progress_frame', 'predict_progressbar', 'predict_status_label']):
            ...
            self.prediction_running = False
            # Clean up comparison data attributes if they exist
        ...
        try:
            while not self.calculation_queue.empty():
                ...
                processed_signals += 1
                if signal is None: # None indicates an error in a worker
                    errors_signalled += 1
        except queue.Empty:
            pass # No messages this cycle
        except Exception as q_err:
             main_logger.error(f"Error reading prediction queue: {q_err}")


        # --- Update Progress ---
        # Count completed threads (those no longer alive)
        ...
        try:
            ...
            if self.prediction_running: # Only update status text if still running
                 ...
                 # Reapply stylesheet if needed for object name change
        ...
        if completed_threads == total_threads:
            ...
            if self.prediction_timer.isActive(): self.prediction_timer.stop() # Stop the timer
            self.prediction_running = False # Mark as not running

            # Final UI update for progress bar/status
            final_status_text = ""
            ...
            try:
                ...
                self.predict_status_label.style().unpolish(self.predict_status_label)
                self.predict_status_label.style().polish(self.predict_status_label)
                # Optionally hide progress bar after a short delay or keep it full
                # QTimer.singleShot(2000, lambda: self.predict_progress_frame.setVisible(False))
            except Exception: pass # Ignore UI errors on final update
            ...
            if not collected_results:
                ...
                # Clean up comparison data attributes
                if hasattr(self, '_next_day_actual_result'): del self._next_day_actual_result
                if hasattr(self, '_next_day_actual_date'): del self._next_day_actual_date
                return

            # Combine scores (using the identical logic function)
            ...
            # Prepare list for display (convert to int, handle missing, sort)
            final_scores_list = []
            try:
            ...
            # Clean up comparison data attributes after use
            if hasattr(self, '_next_day_actual_result'): del self._next_day_actual_result
            if hasattr(self, '_next_day_actual_date'): del self._next_day_actual_date
    ...
    def combine_algorithm_scores(self, intermediate_results: dict) -> dict:
        ...
        # (Identical Logic to Tkinter version, but reads weights from PyQt UI elements)
        if not intermediate_results:
            main_logger.warning("combine_algorithm_scores called with no intermediate results.")
            # Return base score for all numbers if no results
        ...
        BASE_SCORE = 100.0 # Starting score before applying deltas
        # Initialize combined deltas (changes from base score)
        combined_deltas = {f"{i:02d}": 0.0 for i in range(100)}
        valid_algo_count = 0 # Count algorithms that returned valid data
        algorithms_processed = [] # Keep track of which algos were processed
        ...
        for algo_name, raw_scores_dict in intermediate_results.items():
            ...
            valid_algo_count += 1
            processed_scores_dict = copy.deepcopy(raw_scores_dict) # Work on a copy

            # --- Apply Weight Factor (Read from PyQt Widgets) ---
            weight_factor = 1.0
            apply_weight = False
            algo_ui_data = self.algorithms.get(algo_name) # Get UI data from main app's dict
            ...
            if algo_ui_data:
                ...
                # Check if main algo and weight are enabled via UI widgets
                    ...
                    if weight_entry:
                        ...
                        if self._is_valid_float_str(weight_str):
                            try:
                                ...
                                apply_weight = True # Mark that weight should be applied
                            except ValueError:
            ...
            if apply_weight and weight_factor != 1.0:
                ...
                temp_scores = {} # Store weighted scores
                num_multiplied = 0
                for num_str, delta_val in processed_scores_dict.items():
                    # Only apply weight to valid numeric deltas for 00-99
                    if isinstance(num_str, str) and len(num_str) == 2 and num_str.isdigit() and isinstance(delta_val, (int, float)):
                        try:
                            ...
                            temp_scores[num_str] = float(delta_val) # Fallback to original delta
                    else:
                        # Keep non-numeric/invalid items as is (though they shouldn't affect final sum)
                        # temp_scores[num_str] = delta_val # Or skip them entirely
                        pass
                processed_scores_dict = temp_scores # Use the weighted scores
                main_logger.debug(f"Weighted scores calculated for {num_multiplied} numbers from '{algo_name}'.")
            # --- End Apply Weight Factor ---

            # --- Add Deltas to Combined Scores ---
            numbers_processed_in_algo = 0
            errors_in_algo = 0
            for num_str, delta_val in processed_scores_dict.items():
                # Ensure key is valid ('00'-'99') and value is numeric before adding
                if isinstance(num_str, str) and len(num_str) == 2 and num_str.isdigit() and isinstance(delta_val, (int, float)):
                    try:
                        delta_float = float(delta_val) # Ensure value is float
                        ...
                        continue # Skip adding invalid delta
                else:
                     # Log only if key was intended to be a number but failed format check
        ...
        # Add combined deltas to the base score
    ...
    def display_prediction_results_qt(self, sorted_predictions, next_day_actual_results, next_day_date):
        ...
        try:
            dialog = QDialog(self) # Parent to main window
            dialog.setWindowTitle(f"Kết quả dự đoán - Ngày {self.selected_date:%d/%m/%Y}")
            # Increased size slightly for better spacing
            dialog.resize(1250, 900) # Adjust size as needed
            dialog.setMinimumSize(900, 700)
            dialog.setModal(False) # Allow interaction with main window
            flags = dialog.windowFlags()
            # Loại bỏ cờ Context Help Button Hint bằng phép toán AND với NOT bitwise
            flags &= ~Qt.WindowContextHelpButtonHint
            # Đặt lại cờ đã sửa đổi cho cửa sổ
            ...
            # --- Grid Display using QScrollArea and QGridLayout ---
            scroll_area = QScrollArea()
            scroll_area.setWidgetResizable(True)
            scroll_area.setStyleSheet("QScrollArea { border: none; }") # No border for scroll area itself

            grid_container_widget = QWidget() # Widget to hold the grid layout
            scroll_area.setWidget(grid_container_widget)
            table_layout = QGridLayout(grid_container_widget)
            table_layout.setSpacing(4) # Spacing between cells


            # Get actual numbers and special number for highlighting
            actuals, spec = set(), -1
            ...
            # --- Cell Styles ---
            # Use object names set here for styling via main stylesheet if preferred,
            # or apply direct styles here. Direct styles are simpler for this case.
            ...
            for idx, (num, score) in enumerate(sorted_predictions):
                ...
                # Determine cell style and content style
                cell_frame = QFrame()
                cell_layout = QVBoxLayout(cell_frame)
                cell_layout.setContentsMargins(5, 5, 5, 5) # Padding inside cell
                cell_layout.setSpacing(1) # Spacing between number and score
                ...
                # Apply direct stylesheet to the frame for background/border
                ...
                label_num.setStyleSheet(f"color: {current_num_color}; background-color: transparent;") # Transparent BG
                ...
                # Add labels to cell layout
                cell_layout.addWidget(label_num)
                cell_layout.addWidget(label_score)

                # Add cell frame to the main grid layout
                table_layout.addWidget(cell_frame, row, col)
                # Set row/column stretch policy for resizing
                table_layout.setRowStretch(row, 1)
                table_layout.setColumnStretch(col, 1)


            main_layout.addWidget(scroll_area, 1) # Add scroll area with stretch factor
            ...
            legend_layout.setVerticalSpacing(8) # More vertical space for legend items

            legend_layout.addWidget(QLabel("<b>Chú thích:</b>"), 0, 0, 1, 3) # Span 3 columns
            ...
            legend_layout.addWidget(hit_color_box, 1, 0, Qt.AlignTop) # Align color box top
            ...
            legend_layout.setColumnStretch(2, 1) # Add stretch to push legend left
            legend_layout.setRowStretch(3, 1) # Add stretch below legend items
            ...
            close_button.clicked.connect(dialog.accept) # Close dialog on click
            close_button_layout.addWidget(close_button)
            close_button_layout.addStretch(1)
            main_layout.addWidget(close_button_widget)

            dialog.show() # Show the results window
    ...
    # --- Performance Calculation (PyQt5 Adaptation) ---
    ...
    def calculate_combined_performance(self):
        ...
        try:
            ...
            # Performance range must be within the loaded data range
        ...
        try:
            ...
            for i, r in enumerate(sorted_results_for_cache):
                hist_cache[r['date']] = sorted_results_for_cache[:i] # History before date r
            main_logger.debug(f"History cache built: {len(hist_cache)} entries.")

            # Determine valid prediction dates within the selected range
            ...
            valid_predict_dates = [
                ...
                # Need history for the predict date AND results for the day after
            ...
            date_range_str_for_status = f"{start_s} - {end_s}" # For display
        ...
        # --- Start Worker Thread and UI ---
        self.performance_calc_running = True # Set flag
        self.perf_calc_button.setEnabled(False) # Disable button
        ...
        try:
            ...
            self.perf_status_label.setObjectName("ProgressRunning") # Style
            self.perf_status_label.style().unpolish(self.perf_status_label) # Reapply style
        ...
        # Start the QTimer to check the performance queue
    ...
    def _performance_worker(self, active_instances, results_map, history_cache,
        ...
        # (Identical to the previously provided and corrected Tkinter version's worker logic)
        # It uses self.perf_queue.put() to send 'progress', 'finished', or 'error' messages.
            ...
            for i, predict_dt in enumerate(predict_dates_list):
                try:
                    ...
                    for name, inst in active_instances.items():
                        try:
                            # perf_logger.debug(f"Predicting with {name} for {predict_dt}")
                    ...
                    # perf_logger.debug(f"Combining scores for {predict_dt}")
                    # Use the main app's combine function - crucial for consistent weight application
    ...
    def _check_perf_queue(self):
        ...
        try:
            while not self.perf_queue.empty(): # Process all messages
                ...
                if msg_type == "progress":
                    ...
                    try: # Reset UI
                        ...
                        self.perf_status_label.style().unpolish(self.perf_status_label)
                        self.perf_status_label.style().polish(self.perf_status_label)
                        self.perf_progress_frame.setVisible(False) # Hide progress frame on error
                    except Exception: pass
                    ...
                    try: # Final UI update
                        self.perf_calc_button.setEnabled(True)
                        start_s = self.perf_start_date_edit.text() # Get final range from UI
                        end_s = self.perf_end_date_edit.text()
        ...
        # QTimer handles rescheduling automatically if not stopped
    ...
    def extract_numbers_from_result_dict(self, result_dict: dict) -> set:
        ...
        numbers = set()
        # Keys to ignore when extracting numbers
        keys_to_ignore = {'date','_id','source','day_of_week','sign','created_at','updated_at','province_name','province_id'}
        ...
        for key, value in result_dict.items():
            ...
            elif value is not None: # Handle single values (int, str)
            ...
            for item in values_to_check:
                ...
                try:
                    ...
                    num = -1
                    # Prioritize last 2 digits if they are numeric
                    if len(s_item) >= 2 and s_item[-2:].isdigit():
                        num = int(s_item[-2:])
                    # Handle single digit strings that are numeric
                    elif len(s_item) == 1 and s_item.isdigit():
                        num = int(s_item)
                    # Check if the number is within the valid range (0-99)
                    if 0 <= num <= 99:
                        numbers.add(num)
                except (ValueError, TypeError):
                    # Ignore items that cannot be converted or processed
                    pass
        return numbers
    ...
    def update_status(self, message: str):
        ...
        # Determine status type for potential styling
        ...
        if hasattr(self, 'status_bar_label'):
            ...
            # Set custom property for styling via stylesheet
            self.status_bar_label.setProperty("status", status_type)
            # Re-polish to apply property-based style changes
    ...
    def closeEvent(self, event):
        ...
        # Ask for confirmation if optimizer is running
        if hasattr(self, 'optimizer_app_instance') and self.optimizer_app_instance and self.optimizer_app_instance.optimizer_running:
            ...
            if reply == QMessageBox.Yes:
                ...
                try:
                     ...
                     # Give optimizer a moment to process stop? Maybe not necessary if daemon
        ...
        # Stop any running timers explicitly
...
def main():
    ...
    # Pass sys.argv to allow command-line arguments if needed by Qt
    # Handle High DPI scaling BEFORE creating QApplication if possible
    try:
        # Try Qt's recommended approach first
    ...
    app.setOrganizationName("LuviDeeZ") # Optional
    ...
    try:
        ...
        # main_window.show() is called at the end of its __init__
        ...
        traceback.print_exc()
        # Use QMessageBox for error display (requires QApplication instance)
        QMessageBox.critical(
            None, # No parent window available if main window failed
            "Lỗi Nghiêm Trọng",
        ...
        # This block might not be reached if sys.exit() is called inside try/except
        main_logger.info("Application shutdown sequence (finally block).")
        # Ensure logging is shutdown properly
        logging.shutdown()


if __name__ == "__main__":
    # --- Pre-run Checks (optional but good practice) ---
    ...
    # Call the main function to initialize and run the application
    main()

    # This log might not be reached if sys.exit() occurred
